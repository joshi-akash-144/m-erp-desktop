...
<div class="modal fade" id="debit_note_voucher_modal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
            ...
            <div class="modal-body">
                <div class="module-form-section">
                        ...
                        <div class="card-body p-2">
                            <div class="row">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <span id="purchase_order_serial_wrap" class="fs-4 fw-bold text-primary text-end"
                                        style="display:none;">
                                        Purchase Order: <span id="purchase_order_serial" class="text-dark">--</span>
                                    </span>
                                </div>
                    ...
                    <div class="module-page-footer mt-0" id="view_secondary_narration_row" style="{{ $secNarration ? '' : 'display:none;' }}">
                        <span class="fw-bold">Cheque:</span> <span id="view_secondary_narration"
                            class="bg-white text-dark p-1">{{ $secNarration ?: '--' }}</span>
                    </div>
                ...
                @php
                    if (!$customerName && $voucher->details && count($voucher->details) > 0) {
                        $firstDebit = $voucher->details->first(fn($d) => ((float)$d->debit) > 0);
                        if ($firstDebit) {
                                            ...
                                            @if(isset($detail->refs) && count($detail->refs) > 0)
                                                @foreach($detail->refs as $ref)
                                                    @php
                                                        ...
                                                        $pQtyVal = isset($ref['p_qty']) && $ref['p_qty'] !== '' ? (float)$ref['p_qty'] : null;
                                                    @endphp
                                                    <tr>
                                                        ...
                                                        <td>{{ !empty($ref['show_date']) ? \Carbon\Carbon::parse($ref['show_date'])->format('d-m-Y') : '' }}</td>
                                                        <td class="text-center">{{ $drCr }}</td>
                                                        <td class="text-end font-monospace">{{ $qtyVal !== null && function_exists('formatQty') ? formatQty($qtyVal) : $qtyVal }}</td>
                                                        <td class="text-end font-monospace">{{ $pQtyVal !== null && function_exists('formatQty') ? formatQty($pQtyVal) : $pQtyVal }}</td>
                                                        <td class="text-end font-monospace">{{ formatIndianNumber($refAmount) }}</td>
                                                        <td class="text-end font-monospace">{{ !empty($ref['cd']) ? formatIndianNumber($ref['cd']) : '0.00' }}</td>
                                                        <td class="text-end font-monospace">{{ !empty($ref['tds']) ? formatIndianNumber($ref['tds']) : '0.00' }}</td>
                                                        <td class="text-end font-monospace">{{ !empty($ref['premium']) ? formatIndianNumber($ref['premium']) : '0.00' }}</td>
                                                    </tr>
                                                @endforeach
                                            @endif
                                        @endforeach
