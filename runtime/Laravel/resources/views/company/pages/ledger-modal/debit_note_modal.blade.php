<style>
    #debit_note_modal .table-vcenter td,
    #debit_note_modal .table-vcenter th {
        vertical-align: middle;
        padding: 4px 8px;
    }

    #debit_note_modal .grid-label {
        color: #555;
        font-weight: 700;
        font-size: 0.85rem;
    }

    #debit_note_modal .grid-value {
        color: #000;
        font-weight: 800;
        font-size: 0.85rem;
    }

    #debit_note_modal .table-header-grey th {
    ...
    #debit_note_modal .table-bordered td,
    #debit_note_modal .table-bordered th {
        border: 1px solid #ccc;
    }

    #debit_note_modal .bg-grey-cell {
        background-color: #e5e7eb !important;
    }
</style>

<div class="modal fade" id="debit_note_modal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" style="max-width: 1200px;">
        <div class="modal-content border-0 shadow-lg rounded-1">
            ...
            <div class="modal-header bg-light pb-2 pt-1 px-4">
                <h3 class="modal-title text-primary fw-bold m-0" style="font-size: 1.3rem;">Debit Note (Purchase Return)</h3>
            ...
            <div class="modal-body bg-white pt-1 px-4 pb-4">
                ...
                <div class="row gx-3 gy-2 mb-3">
                    <div class="col-2">
                        ...
                        <span class="grid-value ms-1">{{ $debitNote->debit_note_serial ?? '' }}</span>
                    </div>
                    <div class="col-2">
                        <span class="grid-label">Date :</span>
                        <span
                            class="grid-value ms-1">{{ !empty($debitNote->debit_note_date) ? \Carbon\Carbon::parse($debitNote->debit_note_date)->format('d / m / Y') : '' }}</span>
                    </div>
                    <div class="col-2">
                        <span class="grid-label">Ref No :</span>
                        <span class="grid-value ms-1">{{ $debitNote->reference_number ?? '' }}</span>
                    </div>
                    <div class="col-4">
                        <span class="grid-label">Party Name :</span>
                        <span class="grid-value ms-1 text-uppercase">{{ $debitNote->account->name ?? '' }}</span>
                    </div>
                    <div class="col-2">
                        <span class="grid-label">Bill No :</span>
                        <span class="grid-value ms-1 text-uppercase">{{ $debitNote->purchaseInvoice->reference_number ?? '' }}</span>
                    </div>
                    <div class="col-3">
                        <span class="grid-label">Invoice Type :</span>
                        <span class="grid-value ms-1 text-uppercase">{{ $debitNote->purchaseType->name ?? '' }}</span>
                    </div>
                </div>
                ...
                <div class="table-responsive border mb-3">
                    <table class="table table-sm table-bordered table-vcenter text-nowrap mb-0" style="font-size: 0.85rem;">
                        ...
                        <tbody>
                            @if(isset($debitNote) && $debitNote->details)
                ...
                <div class="row mb-3">
                    <div class="col-12 d-flex justify-content-center fw-bold text-secondary gap-5" style="font-size: 0.9rem;">
                        <div>Total Qty : <span class="ms-2 fw-bolder text-dark">
                                ...
                                $totalQty = 0;
                                if(isset($debitNote) && $debitNote->details) {
                                foreach($debitNote->details as $d) {
                        ...
                        <div class="mb-3 row gx-2">
                            <div class="col-12">
                                ...
                                <textarea class="form-control text-dark bg-white" rows="3" disabled
                                    style="resize: none; font-size: 0.750rem !important;">{{ $debitNote->remarks ?? '' }}</textarea>
                            </div>
                        </div>
                        <div class="row mt-2">
                            @if(isset($debitNote->creator->name))
                            <div class="col-6">
                                <span class="fw-bold text-dark" style="font-size: 0.85rem;"><i
                                        class="fa-solid fa-user me-1 text-secondary"></i>Created By:
                                    {{ $debitNote->creator->name }}</span>
                            </div>
                            @endif
                            @if(isset($debitNote->updater->name))
                            <div class="col-6">
                                <span class="fw-bold text-dark" style="font-size: 0.85rem;"><i
                                        class="fa-solid fa-user-pen me-1 text-secondary"></i>Updated By: {{ $debitNote->updater->name }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="col-5">
                        ...
                        <div class="table-responsive border h-100">
                            <table class="table table-sm table-bordered table-vcenter text-nowrap mb-0" style="font-size: 0.85rem;">
                                ...
                                <tbody>
                                    @php
                                    $sundries = isset($debitNote) && $debitNote->billSundries ? $debitNote->billSundries : collect([]);
                                    @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center fw-bold text-dark bg-grey-cell">
                                            {{ !empty($bs->rate_percent) ? number_format($bs->rate_percent, 2) : '' }}</td>
                            ...
                            <div class="d-flex justify-content-end align-items-center mt-3 pe-3 mb-2">
                                ...
                                <span class="fw-bolder text-danger"
                                    style="font-size: 1.2rem;">{{ !empty($debitNote->net_amount) ? formatIndianNumber($debitNote->net_amount, 2) : '0.00' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
