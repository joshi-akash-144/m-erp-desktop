...
<style>
    .cn-filter-row { grid-template-columns: 160px 160px 1fr 2fr; }
    @media (max-width: 992px) { .cn-filter-row { grid-template-columns: 1fr; } }
</style>
@endsection
            ...
            <div class="card border-0 shadow-sm rounded-0">
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-danger rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        @can('credit_note.create')
                            <a href="{{ route('credit-notes.create') }}" class="btn btn-outline-danger btn-sm">
                                <i class="fa-solid fa-plus me-1"></i> New Credit Note
                            </a>
                        @endcan
    ...
    <div class="page-body">
        <div class="container-xl">
            ...
            <div class="card shadow-sm mb-2 p-2">
                <div class="grid-row cn-filter-row row-body">
                    <div class="grid-item">
                    ...
                    {{--  <div class="grid-item">
                        ...
                        <select id="filter_bill_from" class="form-select select2">
                            ...
                            @foreach ($cnSerials as $cn)
                                <option value="{{ $cn->credit_note_serial }}">{{ $cn->credit_note_serial }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="grid-item">
...
<script>
    const cnListUrl    = "{{ route('credit-notes.index') }}";
    const cnCreateUrl  = "{{ route('credit-notes.create') }}";
    const cnEditUrl    = "{{ route('credit-notes.edit') }}";
</script>
<script src="{{ asset('js/utils/icons.js') }}?v={{ filemtime(public_path('js/utils/icons.js')) }}"></script>
<script src="{{ asset('js/modules/credit-note/index.js') }}?v={{ filemtime(public_path('js/modules/credit-note/index.js')) }}"></script>
@endsection
