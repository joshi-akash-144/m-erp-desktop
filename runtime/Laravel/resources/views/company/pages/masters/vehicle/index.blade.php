            ...
            <div class="card border-0 shadow-sm rounded-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fa);">
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex flex-wrap justify-content-md-end justify-content-start gap-2">
                                ...
                                {{-- @canany(['vehicle.print', 'vehicle.export'])
                                <div class="dropdown">
                                    <button
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        @php
                                        $actions = exportActions('vehicle', 'vehicle');
                                        @endphp
                                ...
                                <div class="waves-effect">
                                    <a href="#" class="btn btn-outline-primary btn-sm js-load-modal"
                                        data-module="masters/vehicle/modal" data-init="openVehicleCreateModal">
...
@section('script')
    <script>
        const vehicleCreateUrl = "{{ route('vehicles.create') }}";
        const vehicleListUrl = "{{ route('vehicles.index') }}";
        const vehicleEditUrl = "{{ route('vehicles.edit', ':id') }}";
        const vehicleViewUrl = "{{ route('vehicles.show', ':id') }}";
        const vehicleDeleteUrl = "{{ route('vehicles.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/master/vehicle/index.js') }}?v={{ hash_file('md5', public_path('js/master/vehicle/index.js')) }}"></script>
@endsection
