...
@section('title', 'Dairy Parameter - List')
                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div class="bg-primary-lt text-primary rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 38px; height: 38px;">
                            <i class="fs-3 fa-solid fa-house-chimney-window"></i>
                        </div>
                        <div>
                    ...
                    {{-- Export Dropdown (only for users with print/export permission) --}}
                        @canany(['dairy_parameter.print', 'dairy_parameter.export'])
                            <div class="dropdown">
                                <button
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    @php
                                        $actions = exportActions('dairy-parameters', 'dairy_parameter');
                                    @endphp
                        ...
                        {{-- Add Dairy Parameter Button --}}
                        @can('dairy_parameter.create')
                            <a href="#"
                            class="btn btn-outline-primary btn-sm waves-effect js-load-modal"
                            data-module="masters/dairy-parameter/modal"
                            data-init="openDairyParameterCreateModal">
                    ...
                    <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                        <div class="card border-0 border-start border-4 border-primary">
                            <div class="card-body shadow p-2">
                                <h3 class="page-title font-monospace"><i class="fa-solid fa-house-chimney-window text-primary"></i>&nbsp;Dairy Parameter List</h3>
                            </div>
                        </div>
                    </div>
        ...
        <div class="page-body">
            <div class="container-xl">
                <div class="card">
                    ...
                    <!-- 🔹 Dairy Parameter Table -->
                    <div class="card-body p-0">
                        <div id="dairy_parameter_table">
...
@section('script')
    <script>
        const dairyParameterListUrl = "{{ route('dairy-parameters.index') }}";
        const dairyParameterCreateUrl = "{{ route('dairy-parameters.create') }}";
        const dairyParameterEditUrl = "{{ route('dairy-parameters.edit', ':id') }}";
        const dairyParameterViewUrl = "{{ route('dairy-parameters.show', ':id') }}";
        const dairyParameterDeleteUrl = "{{ route('dairy-parameters.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    
    <script src="{{ asset('js/master/dairy-parameter/index.js') }}?v={{ hash_file('md5', public_path('js/master/dairy-parameter/index.js')) }}"></script>  
@endsection