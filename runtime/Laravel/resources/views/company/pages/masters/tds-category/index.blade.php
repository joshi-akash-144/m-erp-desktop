...
@section('title', 'TDS Category - List')
...
<div class="page-header d-print-none">
        <div class="container-xl">
                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div class="bg-primary-lt text-primary rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 38px; height: 38px;">
                            <i class="fs-3 fa-solid fa-percentage"></i>
                        </div>
                        <div>
           ...
           {{-- Export Dropdown (only for users with print/export permission) --}}
                        @canany(['tds_category.print', 'tds_category.export'])
                            <div class="dropdown">
                                <button
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    @php
$actions = exportActions('tds-categories', 'tds_category');
                                    @endphp
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="card border-0 border-start border-4 border-primary">
                                <div class="card-body shadow p-2">
                                    <h3 class="page-title font-monospace"><i class="fa-solid fa-user-group text-primary"></i>&nbsp;Tds Category List</h3>  
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body border-bottom">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4 col-lg-3">
                                        ...
                                        <select id="group_id" class="form-select tom-select">
                                            <option value="">All Groups</option>
                                            @foreach ($TdsCategories as $TdsCategorie)
                                                <option value="{{ $TdsCategorie->id }}">{{ $TdsCategorie->section }}</option>
                                            @endforeach
                                        </select>
                                    </div>
...
@section('script')   
    <script>
        const tdsCategoryListUrl = "{{ route('tds-categories.index') }}";
        const tdsCategoryCreateUrl = "{{ route('tds-categories.create') }}"; 
        const tdsCategoryViewUrl = "{{ route('tds-categories.show', ':id') }}";       
        const tdsCategoryEditUrl = "{{ route('tds-categories.edit', ':id') }}";       
        const tdsCategoryDeleteUrl = "{{ route('tds-categories.destroy', ':id') }}";            
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/master/tds-category/index.js') }}?v={{ hash_file('md5', public_path('js/master/tds-category/index.js')) }}"></script>   
@endsection