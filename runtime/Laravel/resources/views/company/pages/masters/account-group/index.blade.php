                    ...
                    <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                        ...
                        <div class="d-flex align-items-center gap-3">
                            <div class="bg-primary-lt text-primary rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 38px; height: 38px;">
                                <i class="fs-3 fa-solid fa-users"></i>
                            </div>
                            <div>
                        ...
                        {{-- Export Dropdown (only for users with print/export permission) --}}
                                @canany(['account_group.print', 'account_group.export'])
                                    <div class="dropdown">
                                        <button
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @php
        $actions = exportActions('account-groups', 'account_group');
                                            @endphp
                            ...
                            @can('account_group.create')
                                ...
                                <a href="#"
                                    ...
                                    data-init="openAccountGroupCreateModal">
                            ...
                            <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                                <div class="card border-0 border-start border-4 border-primary">
                                    <div class="card-body shadow p-2">
                                        <h3 class="page-title font-monospace"><i class="fa-solid fa-user-group text-primary"></i>&nbsp;Account Groups List</h3>  
                                    </div>
                                </div>
                            </div>
...
@section('script')   
    <script>
        const accountGroupListUrl = "{{ route('account-groups.index') }}";
        const accountGroupCreateUrl = "{{ route('account-groups.create') }}"; 
        const accountGroupViewUrl = "{{ route('account-groups.show', ':id') }}";       
        const accountGroupEditUrl = "{{ route('account-groups.edit', ':id') }}";        
        const accountGroupDeleteUrl = "{{ route('account-groups.destroy', ':id') }}";            
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/master/account-group/index.js') }}?v={{ hash_file('md5', public_path('js/master/account-group/index.js')) }}"></script>   
@endsection