...
$title = $modalData['title'] ?? 'Add Element';
$uuid = $modalData['uuid'] ?? '';
...
<div class="modal fade master-modal" id="element_modal" tabindex="-1" data-bs-backdrop="static" 
     data-bs-keyboard="false" aria-labelledby="element_modal" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered">
    ...
    <div class="modal-body">
          <form id="element_form"
            action="{{ $formMode === 'edit' ? route('elements.update', $data->id) : route('elements.store') }}"
            ...
            <div class="row g-2">
                ...
                <div class="col-md-12">
                    ...
                    <div class="master-form-section">
                        <div class="master-section-title "><i class="fa-solid fa-atom"></i>&nbsp;
                        Element Details</div>
                        <div class="row g-2">
                            <!-- Element Name -->
                            <div class="col-md-6">
                                <label class="form-label required"> <i class="fa-solid fa-atom text-secondary"></i>&nbsp;Element Name</label>
                                <input type="text" name="name" id="name" class="form-control" placeholder="Enter Element Name"
                            ...
                            <div class="col-6">
                                ...
                                placeholder="Enter Print Name" value="{{ old('print_name', $data->print_name ?? '') }}"
...
$range = config('constants.range');
                            @endphp
                            <div class="col-6">
                                <label class="form-label required"><i class="fa-solid fa-bullseye text-secondary"></i>&nbsp;Range</label>
                                <select name="range" id="range" class="form-select select2" {{ $isView ? 'disabled' : 'required' }}>
                                  <option value="">Select Range...</option>
                                  @foreach ($range as $groupTypeId => $groupType)
                                  <option value="{{ $groupTypeId }}" {{ ($data->range ?? '') == $groupTypeId ? 'selected' : '' }}>
                                    {{ $groupType }}
                                  </option>
                                  @endforeach
                                </select>
      ...
      <div class="modal-footer master-modal-footer">
        ...
        @if(!$isView)
          <button type="submit" form="element_form" class="btn btn-primary form-save-btn waves-effect">
...
<script src="{{ asset('js/master/element/modal.js') }}?v={{ hash_file('md5', public_path('js/master/element/modal.js')) }}"></script>  
