...
@section('title', 'Bill Sundry - List')
                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-2">
                         ...
                         {{-- Export Dropdown (only for users with print/export permission) --}}
                            @canany(['bill_sundry.print', 'bill_sundry.export'])
                                <div class="dropdown">
                                    <button
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        @php
                                    $actions = exportActions('bill-sundries', 'bill_sundry');
                                        @endphp
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="card border-0 border-start border-4 border-primary">
                                <div class="card-body shadow p-2">
                                    <h3 class="page-title font-monospace"><i class="fa-solid fa-bag-shopping text-primary"></i>&nbsp;Bill Sundry List</h3>  
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="filterCollapse">
                            <div class="card-body border-bottom">
                                <div class="row g-3">
                                    ...
                                    <!-- Bill Sundry Type Dropdown -->
                                    <div class="col-md-4 col-lg-3">
                                        <label class="form-label fw-bold">Select Bill Sundry Type</label>
                                        <select id="bill_sundry_type_id" class="form-select select2">
                                            <option value="">All Bill Sundry type</option>
                                            @foreach ($billSundry as $group)
                                                <option value="{{ $group->bill_sundry_type }}">{{ $group->bill_sundry_type }}</option>
                                            @endforeach
                                        </select>
                                    </div>
...
@section('script')  
     <script>
        const billSundryListUrl = "{{ route('bill-sundries.index') }}";
        const billSundryCreateUrl = "{{ route('bill-sundries.create') }}"; 
        const billSundryViewUrl = "{{ route('bill-sundries.show', ':id') }}";       
        const billSundryEditUrl = "{{ route('bill-sundries.edit', ':id') }}";       
        const billSundryDeleteUrl = "{{ route('bill-sundries.destroy', ':id') }}";            
    </script>

    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/master/bill-sundry/index.js') }}?v={{ hash_file('md5', public_path('js/master/bill-sundry/index.js')) }}"></script>
@endsection
