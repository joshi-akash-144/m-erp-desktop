...
@section('title', 'Payee Category - List')
...
<div class="page-header d-print-none">
        <div class="container-xl">
            ...
            {{-- Export Dropdown (only for users with print/export permission) --}}
                        @canany(['payee_category.print', 'payee_category.export'])
                            <div class="dropdown">
                                <button
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    @php
$actions = exportActions('payee-categories', 'payee_category');
                                    @endphp
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="card border-0 border-start border-4 border-primary">
                                <div class="card-body shadow p-2">
                                    <h3 class="page-title font-monospace"><i class="fa-solid fa-user-group text-primary"></i>&nbsp;Payee Category List</h3>  
                                </div>
                            </div>
                        </div>
            ...
            <div class="page-body">
                <div class="container-xl">
                    <div class="card">
                                    ...
                                    <div class="col-md-4 col-lg-3">
                                        ...
                                        <select id="group_id" class="form-select tom-select">
                                            ...
                                            @foreach ($groups as $group)
                                                <option value="{{ $group->id }}">{{ $group->payee_category }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <!-- 🔹 Payee Category Table -->
                        <div class="card-body p-0">
                            <div id="payee_category_table">
...
@section('script')
    <script>
        const payeeCategoryListUrl = "{{ route('payee-categories.index') }}";
        const payeeCategoryCreateUrl = "{{ route('payee-categories.create') }}";
        const payeeCategoryViewUrl = "{{ route('payee-categories.show', ':id') }}";        
        const payeeCategoryEditUrl = "{{ route('payee-categories.edit', ':id') }}";
        const payeeCategoryDeleteUrl = "{{ route('payee-categories.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/master/payee-category/index.js') }}?v={{ hash_file('md5', public_path('js/master/payee-category/index.js')) }}"></script>   
