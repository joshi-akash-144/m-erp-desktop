...
@section('title', 'Tax Category - List')
@section('css')
                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div class="bg-primary-lt text-primary rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 38px; height: 38px;">
                            <i class="fs-3 fa-solid fa-calculator"></i>
                        </div>
                        <div>
                         ...
                         {{-- Export Dropdown (only for users with print/export permission) --}}
                          @canany(['tax_category.print', 'tax_category.export'])
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                @php
                                    $actions = exportActions('tax-categories', 'tax_category');
                                @endphp
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="card border-0 border-start border-4 border-primary">
                                <div class="card-body shadow p-2">
                                    <h3 class="page-title font-monospace"><i class="fa-solid fa-calculator text-primary"></i>&nbsp;Tax Category List</h3>  
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="filterCollapse">
                            <div class="card-body border-bottom">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4 col-lg-3">
                                        <label class="form-label fw-bold">Select Tax Category</label>
                                        <select id="tax_category_id" class="form-select tom-select">
...
@section('script')
    ...
    <script>
        const taxCategoryListUrl = "{{ route('tax-categories.index') }}";
        const taxCategoryCreateUrl = "{{ route('tax-categories.create') }}"; 
        const taxCategoryViewUrl = "{{ route('tax-categories.show', ':id') }}";       
        const taxCategoryEditUrl = "{{ route('tax-categories.edit', ':id') }}";        
        const taxCategoryDeleteUrl = "{{ route('tax-categories.destroy', ':id') }}";            
    </script>

    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>    
    <script src="{{ asset('js/master/tax-category/index.js') }}?v={{ hash_file('md5', public_path('js/master/tax-category/index.js')) }}"></script>
@endsection
