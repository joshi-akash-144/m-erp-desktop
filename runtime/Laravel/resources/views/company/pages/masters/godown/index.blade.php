                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-2">
                      {{-- Export Dropdown (only for users with print/export permission) --}}
                                        @canany(['godown.print', 'godown.export'])
                                            <div class="dropdown">
                                                <button
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    @php
                                                        $actions = exportActions('godowns', 'godown');
                                                    @endphp
                                        ...
                                        <div class="waves-effect">
                                            ...
                                            data-module="masters/godown/modal"
                                            data-init="openGodownCreateModal">
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="card border-0 border-start border-4 border-primary">
                                <div class="card-body shadow p-2">
                                    <h3 class="page-title font-monospace"><i class="fa-solid fa-warehouse text-primary"></i>&nbsp;Godown List</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="filterCollapse">
                            <div class="card-body border-bottom">
                                <div class="row g-3">
                                    ...
                                    <!-- Group Dropdown -->
                                    <div class="col-md-4 col-lg-3">
                                        <label class="form-label fw-bold">Select Godown</label>
                                        <select id="group_id" class="form-select tom-select">
                                            <option value="">All Godown</option>
                                            @foreach ($groups as $group)
                                                <option value="{{ $group->id }}">{{ $group->godown_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
...
@section('script')
    <script>
        const godownListUrl = "{{ route('godowns.index') }}";
        const createRouteUrl = "{{ route('godowns.create') }}";
        const editRouteUrl = "{{ route('godowns.edit', ':id') }}";
        const viewRouteUrl = "{{ route('godowns.show', ':id') }}";
        const deleteRouteUrl = "{{ route('godowns.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    
    <script src="{{ asset('js/master/godown/index.js') }}?v={{ hash_file('md5', public_path('js/master/godown/index.js')) }}"></script>  
@endsection