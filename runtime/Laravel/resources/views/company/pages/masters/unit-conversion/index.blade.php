...
@section('title', 'Unit Conversion - List')
@section('css')
                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div class="bg-primary-lt text-primary rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 38px; height: 38px;">
                            <i class="fs-3 fa-solid fa-scale-balanced"></i>
                        </div>
                        <div>
                    ...
                    {{-- Add Unit Button --}}
                    @can('unit_conversion.create')
                       <a href="#" class="btn btn-outline-primary btn-sm js-load-modal"
                            data-module="masters/unit-conversion/modal" data-init="openUnitConversionCreateModal">
                        ...
                        <div class="row g-2 align-items-center justify-content-between">
                            ...
                            <div class="card border-0 border-start border-4 border-primary">
                                <div class="card-body shadow p-2">
                                    <h3 class="page-title font-monospace"><i class="fa-solid fa-scale-balanced text-primary"></i>&nbsp;Unit Conversion List</h3>
                                </div>
                            </div>
                            </div>
                                ...
                                <div class="card border-0 border-end border-4 border-primary">
                                    <div class="card-body shadow-sm p-2">
                                        <div class="d-flex flex-wrap justify-content-md-end justify-content-start gap-2">
                                            ...
                                            {{-- @canany(['unit_conversion.print', 'unit_conversion.export'])
                                                <div class="dropdown">
                                                    <button
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                        @php
                                                            $actions = exportActions('unit-conversions', 'unit_conversion');
                                                        @endphp
                                    ...
                                    <div class="card-body border-bottom">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-4 col-lg-3">
                                                ...
                                                <select id="unit_conversion_id" class="form-select tom-select">
                                                    <option value="">All Unit Conversion</option>
                                                    @foreach ($unitConversions as $unitConversion)
                                                        <option value="{{ $unitConversion->id }}">{{ $unitConversion->mainUnit->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
...
@section('script')
    <script>
        const unitConversionListUrl = "{{ route('unit-conversions.index') }}";
        const unitConversionCreateUrl = "{{ route('unit-conversions.create') }}";
        const unitConversionEditUrl = "{{ route('unit-conversions.edit', ':id') }}";
        const unitConversionViewUrl = "{{ route('unit-conversions.show', ':id') }}";
        const unitConversionDeleteUrl = "{{ route('unit-conversions.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    
    <script src="{{ asset('js/master/unit-conversion/index.js') }}?v={{ hash_file('md5', public_path('js/master/unit-conversion/index.js')) }}"></script>  
@endsection