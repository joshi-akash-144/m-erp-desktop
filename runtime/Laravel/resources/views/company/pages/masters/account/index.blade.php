        ...
        <div class="container-xl">
            <div class="card border-0 shadow-sm rounded-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fa);">
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    @canany(['account.print', 'account.export'])
                    <div class="dropdown">
                        <button
                    ...
                    @endcanany
                        ...
                        @can('account.create')
                            {{-- Add Account Button --}}
                            <a href="#" class="btn btn-outline-primary btn-sm js-load-modal waves-effect"
                                data-module="masters/account/modal" data-init="openAccountCreateModal">
                    ...
                    <div class="card border-0 border-start border-4 border-primary">
                        <div class="card-body shadow p-2">
                            <h3 class="page-title font-monospace"><i
                                    class="fa-solid fa-money-check text-primary"></i>&nbsp;Account List</h3>
                        </div>
                    </div>
                </div>
                ...
                <div id="filterCollapse">
                    <div class="card-body border-bottom">
                        <div class="row g-3">
                            ...
                            <!-- Part Type Dropdown -->
                            <div class="col-md-4 col-lg-3">
                                <label class="form-label fw-bold">Select Party Type</label>
                                <select id="party_type_id" class="form-select tom-select">
                                    <option value="">All Party Types</option>
                                    @foreach ($partyTypes as $partyKey => $partyType)
                                    <option value="{{ $partyKey }}">{{ $partyType }}</option>
                                    @endforeach
                                </select>
                            </div>
                         ...
                         <div class="col-lg-3">
                            ...
                            <select name="account_group_id" id="account_group_id_index" class="form-select tom-select">
...
@section('script')
    <script>
        const accountCreateUrl = "{{ route('accounts.create') }}";
        const accountListUrl = "{{ route('accounts.index') }}";
        const accountEditUrl = "{{ route('accounts.edit', ':id') }}";
        const accountViewUrl = "{{ route('accounts.show', ':id') }}";
        const accountDeleteUrl = "{{ route('accounts.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/master/account/index.js') }}?v={{ hash_file('md5', public_path('js/master/account/index.js')) }}"></script>
@endsection
