                ...
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        {{-- Export Dropdown (only for users with print/export permission) --}}
                             @canany(['destination.print', 'destination.export'])
                                <div class="dropdown">
                                    <button
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        @php
                                            $actions = exportActions('destinations', 'destination');
                                        @endphp
                            ...
                            @can('destination.create')
                                ...
                                data-module="masters/destination/modal"
                                data-init="openDestinationCreateModal">
                            ...
                            <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                                <div class="card border-0 border-start border-4 border-primary">
                                    <div class="card-body shadow p-2">
                                        <h3 class="page-title font-monospace"><i class="fa-solid fa-location-dot text-primary"></i>&nbsp;Destination List</h3>                            
                                    </div>
                                </div>
                            </div>
...
@section('script')
    <script>
        const destinationListUrl = "{{ route('destinations.index') }}";
        const destinationCreateUrl = "{{ route('destinations.create') }}";
        const destinationEditUrl = "{{ route('destinations.edit', ':id') }}";
        const destinationViewUrl = "{{ route('destinations.show', ':id') }}";
        const destinationDeleteUrl = "{{ route('destinations.destroy', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    
    <script src="{{ asset('js/master/destination/index.js') }}?v={{ hash_file('md5', public_path('js/master/destination/index.js')) }}"></script>  
@endsection