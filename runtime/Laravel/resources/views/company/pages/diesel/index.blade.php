...
@section('title', 'Diesel Expense Register')
...
@section('css')
    ...
    <style>
        ...
        /* Parent voucher row */
        /* #diesel_register_table .tabulator-row.row-parent {
            background-color: #1e3a5f !important;
            color: #fff !important;
            font-weight: 600;
        }
        #diesel_register_table .tabulator-row.row-parent:hover {
            background-color: #24467a !important;
        }
        #diesel_register_table .tabulator-row.row-parent .tabulator-cell {
            color: #fff !important;
            border-right-color: #2d4f70 !important;
        } */

        /* Total row */
        /* #diesel_register_table .tabulator-row.row-total {
            background-color: #e3eaf2 !important;
            font-weight: 700;
        }
        #diesel_register_table .tabulator-row.row-total .tabulator-cell {
            border-right-color: #9aa5b1 !important;
        } */

        /* Item row */
        /* #diesel_register_table .tabulator-row.row-item {
            background-color: #fff;
        }
        #diesel_register_table .tabulator-row.row-item:hover {
...
<form action="">
            ...
            <div class="card">
                <div class="card-body p-0">
                    <div id="diesel_register_table"></div>
                </div>
            </div>
        ...
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white py-2">
                <h5 class="modal-title fw-bold">
                    <i class="fa-solid fa-file-invoice-dollar me-2"></i> Diesel Expense Detail
                </h5>
            ...
            <div class="modal-body bg-light p-3 position-relative" style="min-height: 400px;">
                ...
                <div id="view_modal_content" class="d-none">
                    <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
                        <h6 class="fw-bold m-0 text-nowrap"><i class="fa-solid fa-list-check me-1 text-secondary"></i>Diesel Details</h6>
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-body p-0">
                            <div id="diesel_detail_table"></div>
                        </div>
                    </div>
...
@section('script')
<script>
    const dieselListUrl          = "{{ route('diesel.list') }}";
    const dieselRegisterPrintUrl = "{{ route('diesel.register-print') }}";
    const dieselPrintUrl         = "{{ route('diesel.print', ':id') }}";
    const dieselShowUrl          = "{{ route('diesel.show', ':id') }}";
    const dieselDeleteUrl        = "{{ route('diesel.destroy', ':id') }}";
</script>
<script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
<script src="{{ asset('js/modules/diesel/index.js') }}?v={{ hash_file('md5', public_path('js/modules/diesel/index.js')) }}"></script>
@endsection
