...
@section('title', 'Salary Voucher – Edit #' . ($multiExpenseVoucher->voucher?->voucher_serial ?? $multiExpenseVoucher->id))
            ...
            <div class="card border-0 shadow-sm rounded-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fa);">
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-warning rounded-0">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <a href="{{ route('salary-module.index') }}"
                        ...
                        <div class="grid-item">
                            ...
                            <select name="month" id="month" class="form-select">
                                ...
                                @foreach(['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] as $m)
                                    <option value="{{ $m }}" {{ ($multiExpenseVoucher->month ?? '') == $m ? 'selected' : '' }}>{{ $m }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <div style="max-height: 500px; overflow-x: auto; overflow-y: auto; position: relative;">
                                <table class="table border border-1 border-dark-subtle table-bordered mb-0"
                                    ...
                                    <colgroup>
                                        ...
                                        <col style="width: 500px">    {{-- Account Name --}}
                                        <col style="width: 500px">    {{-- Particular --}}
                                        <col style="width: 200px">    {{-- Amount --}}
                                        <col style="width: 500px">    {{-- Remark --}}
                                    </colgroup>
...
@section('script')
<script>
    const updateSalaryModuleVoucherUrl = "{{ route('salary-module.update', $multiExpenseVoucher->id) }}";
    const getSalaryModuleVoucherDataUrl = "{{ route('salary-module.data', $multiExpenseVoucher->id) }}";
</script>
<script src="{{ asset('js/modules/salary-module/edit.js') }}?v={{ time() }}"></script>
@endsection
