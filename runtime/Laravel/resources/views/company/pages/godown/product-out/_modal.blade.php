                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <small class="text-warning fw-semibold">
                            <i class="fa-solid fa-clock me-1"></i> Loaded from Weighbridge
                        </small>
                    </div>
                </div>
            ...
            <div class="modal-body p-0">
                <table class="table table-hover table-sm align-middle mb-0" id="vehicle_selection_table" autocomplete="off">
                    ...
                    <tbody id="vehicle_list_body">
                        @if(isset($vehicles) && $vehicles->count() > 0)
                            @foreach($vehicles as $vehicle)
                                @php
                                    ...
                                    $netWeight = ($vehicle->gross_weight ?? 0) - ($vehicle->tare_weight ?? 0);
                                @endphp
                        <tr class="selectable-row" 
                                data-id="{{ $vehicle->id }}"
                                data-vehicle="{{ $vehicle->vehicle_number }}" 
                                data-grn-id="{{ $vehicle->grn_id }}"
                                data-grn-serial="{{ $vehicle->grn->grn_serial ?? '' }}" 
                                data-challan-serial="{{ $vehicle->grn->grn_serial ?? '' }}"
                                data-so-id="{{ $vehicle->dairy_po ?? '' }}"
                                data-godown-unit-id="{{ $vehicle->godown_unit_id }}"
                                data-challan-weight="{{ $vehicle->challan_weight ?? 0 }}"
                                data-challan-bags="{{ $vehicle->challan_bags ?? 0 }}"
                                data-bag-qty="{{ $vehicle->bag_count ?? 0 }}"
                                data-grn-type="{{ strtoupper($vehicle->gst_type ?? '') }}"
                                data-date-in="{{ $vehicle->date_in ? (is_string($vehicle->date_in) ? $vehicle->date_in : $vehicle->date_in->format('Y-m-d')) : '' }}"
                                data-time-in="{{ $vehicle->time_in }}"
                                data-created-at="{{ $vehicle->created_at }}"
                                ...
                                data-bag-count="{{ $vehicle->bag_count }}"
                                ...
                                data-challan="{{ $vehicle->reference_number }}"
                                ...
                                <td class="text-center">{{ $vehicle->grn->grn_serial ?? 'N/A' }}</td>
                        ...
                        <!-- NO DATA ROW -->
                        <tr id="vehicle_no_data_row" class="text-center {{ isset($vehicles) && $vehicles->count() > 0 ? 'd-none' : '' }}">
