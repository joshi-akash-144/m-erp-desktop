@php
    $storeRoute  = route('debit-notes.store');
@endphp

<form action="{{ $storeRoute }}" id="debit_note_form" autocomplete="off" data-form-mode="{{ $formMode }}">
    @if ($formMode === 'create')
    ...
    <div class="module-form-section section-danger border-danger">
        ...
        <div class="grid-row dn-row-1 row-body">
            ...
            <div class="grid-item">
                ...
                @if ($formMode === 'edit')
                    <select name="debit_note_id" id="debit_note_id" class="form-select select2">
                        <option value="">--Select DN--</option>
                        @foreach ($dnSerials as $dn)
                            <option value="{{ $dn->id }}" @selected($dn->id == ($debitNoteId ?? null))>
                                {{ $dn->debit_note_serial }}
                            </option>
                        @endforeach
                    </select>
                @else
                    <input type="text" class="form-control bg-primary-lt fw-bold text-dark"
                        value="{{ $dnInfo->serial ?? '' }}" disabled>
                @endif
            </div>
            ...
            <div class="grid-item">
                ...
                <input type="text" name="debit_note_date" id="debit_note_date"
            ...
            <div class="grid-item">
                ...
                <input type="text" name="reference_number" id="reference_number"
                    class="form-control" placeholder="Ref No." value="{{ isset($dnInfo) ? 'DRN-' . $dnInfo->serial : '' }}">
            </div>
                ...
                <select name="account_id" id="account_id" class="form-select">
                    ...
                    @foreach ($suppliers as $supplier)
                        <option value="{{ $supplier['id'] }}">
                            {{ $supplier['name'] }}{{ !empty($supplier['city']) ? ' - ' . $supplier['city'] : '' }}
                        </option>
                    @endforeach
                </select>
            </div>

            {{-- Bill No. (Purchase Invoice reference) --}}
            <div class="grid-item">
                <label class="form-label">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="submit" class="btn btn-danger px-4" id="save_btn">
                            <i class="fa-solid fa-floppy-disk me-1"></i> {{ $formMode === 'edit' ? 'Update' : 'Save' }}
                        </button>
                    </div>
                </div>
            </div>
