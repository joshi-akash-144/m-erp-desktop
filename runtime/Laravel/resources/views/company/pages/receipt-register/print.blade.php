...
<body>
    @php
        ...
        $totalAmount  = $rows->sum('received_amount');
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $row->voucher_date ? \Carbon\Carbon::parse($row->voucher_date)->format('d/m/Y') : '' }}</td>
                <td class="text-center">{{ $row->voucher_serial ?? '' }}</td>
                <td>{{ $row->party_name ?? '' }}</td>
                <td>{{ $row->narration ?? '' }}</td>
                <td class="text-end">{{ number_format($row->received_amount ?? 0, 2) }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
