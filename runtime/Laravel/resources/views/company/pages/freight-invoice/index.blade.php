...
@section('title', 'Freight Invoice – List')
                ...
                <div class="card border-0 shadow-sm rounded-0"
                    ...
                    <div
                        ...
                        <div class="d-flex align-items-center gap-2">
                            @canany(['freight_invoice.print', 'freight_invoice.export'])
                            <div class="dropdown">
                                <button
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    @php
                                    $actions = exportActions('freight-invoice', 'freight_invoice');
                                    @endphp
                            ...
                            <div class="d-flex gap-2">
                                @can('freight_invoice.create')
                                    <a href="{{ route('freight-invoice.create') }}"
...
@section('script')
    ...
    <script>
        const freightInvoiceListUrl = "{{ route('freight-invoice.index') }}";
        const freightEditUrl = "{{ route('freight.edit', ':id') }}";
        const freightPrintUrl = "{{ route('freight-invoice.print-invoice', ':id') }}";
        const freightZonePrintUrl = "{{ route('freight-invoice.print-zonewise', ':id') }}";
        const freightZoneExportUrl = "{{ route('freight-invoice.export-zonewise', ':id') }}";
    </script>
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script
        src="{{ asset('js/modules/freight-invoice/index.js') }}?v={{ hash_file('md5', public_path('js/modules/freight-invoice/index.js')) }}"></script>
