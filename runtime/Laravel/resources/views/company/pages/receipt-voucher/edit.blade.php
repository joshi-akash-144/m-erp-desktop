...
@section('title', 'Receipt Voucher Edit')
@section('css')
    ...
    <div class="page-body">
        <div class="container-xl">
            <div class="card p-2 shadow-sm">
                @include('company.pages.receipt-voucher._form', ['formMode' => 'edit'])
            </div>
        </div>
    </div>
</div>
@include('company.pages.receipt-voucher._ref_modal')
@include('company.pages.receipt-voucher._pending_ref_modal')
@endsection
<script>
    ...
    const storeReceiptVoucher      = "{{ route('receipt-vouchers.store') }}";
    const indexReceiptVoucher      = "{{ route('receipt-vouchers.index') }}";
    const editReceiptVoucher       = "{{ route('receipt-vouchers.edit') }}";
    const updateReceiptVoucher     = "{{ url('/vouchers/receipt-vouchers') }}";
</script>

@section('script')
<script src="{{ asset('js/modules/receipt-voucher/create.js') }}?v={{ hash_file('md5', public_path('js/modules/receipt-voucher/create.js')) }}"></script>
<script src="{{ asset('js/modules/receipt-voucher/edit.js') }}?v={{ hash_file('md5', public_path('js/modules/receipt-voucher/edit.js')) }}"></script>
@endsection
