...
@section('title', 'ONAC Payment Payable – List')
@section('css')
<link rel="stylesheet"
...
@include('company.pages.onac-payment._onac_payment_list_modal')
@endsection
@section('script')
<script>
    const getPaymentPayableDataUrl = "{{ route('onac-payment.payables') }}";
    const closingBalanceUrl = "{{ route('account-balance.closing') }}";
    const getAdvanceListUrl = "{{ route('onac-payment.get-advance-list') }}";
...
<script src="{{ asset('js/modules/onac-payment/onac-payment.js') }}?v={{ hash_file('md5', public_path('js/modules/onac-payment/onac-payment.js')) }}"></script>
@endsection