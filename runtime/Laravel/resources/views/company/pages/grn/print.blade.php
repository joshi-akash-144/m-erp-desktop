            ...
            <table class="report-table" style="width: auto;">
                ...
                <tbody>
                    @forelse($grns as $index => $grn)
                        @foreach($grn->details as $detailIndex => $detail)
                            @php
                                $rowCountOnPage++;
                            ...
                            <tr class="{{ $isFirstDetail ? 'po-first-row' : '' }}">
                                ...
                                {{-- Grn Serial --}}
                                <td class="text-end" style="width: {{ $tableConfig['columns'][3]['width'] ?? 'auto'}};">
                                    {{ $isFirstDetail ? Str::limit($grn->reference_number) : '' }}
                                </td>
                                {{-- purchase_Order_Serial --}}
                                <td class="text-end text-nowrap" style="width: {{ $tableConfig['columns'][4]['width'] ?? 'auto'}};">
                                    {{ $isFirstDetail ? Str::limit($detail->purchase_order_serial) : '' }}
                                </td>
                                {{-- Account Name --}}
                                <td class="text-start" style="width: {{ $tableConfig['columns'][5]['width'] ?? 'auto'}};">
                                    {{ $isFirstDetail ? Str::limit($grn->account->name ?? '', 24) : '' }}
                                </td>
                                {{-- Item Name (Item specific, always show) --}}
                                <td class="text-start" style="width: {{ $tableConfig['columns'][6]['width'] ?? 'auto'}};">
                                    {{ Str::limit($detail->item->name ?? '', 15) }}
                                </td>
                                {{-- Destination Name --}}
                                <td class="text-start" style="width: {{ $tableConfig['columns'][7]['width'] ?? 'auto'}};">
                                    {{ Str::limit($detail->destination->name ?? '', 13) }}
                                </td>
                                {{-- Quantity --}}
                                <td class="text-end text-nowrap" style="width: {{ $tableConfig['columns'][8]['width'] ?? 'auto'}};">
                                    {{ Str::limit(number_format($detail->quantity, 3) ?? '') }}
                                </td>
                                {{-- Party Quantity --}}
                                <td class="text-end text-nowrap" style="width: {{ $tableConfig['columns'][9]['width'] ?? 'auto'}};">
                                    {{ Str::limit(number_format($detail->party_quantity, 3) ?? '') }}
                                </td>
                                {{-- Rate --}}
                                <td class="text-end text-nowrap" style="width: {{ $tableConfig['columns'][10]['width'] ?? 'auto'}};">
                                    {{ Str::limit(number_format($detail->rate, 2) ?? '') }}
                                </td>
                                {{-- Inclusive Rate --}}
                                <td class="text-end text-nowrap" style="width: {{ $tableConfig['columns'][11]['width'] ?? 'auto'}};">
                                    {{ Str::limit(number_format($detail->inclusive_rate, 2) ?? '') }}
                                </td>
                            </tr>
                        @endforeach
                    @empty
