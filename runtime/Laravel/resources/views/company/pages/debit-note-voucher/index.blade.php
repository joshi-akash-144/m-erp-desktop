...
@section('title', 'Debit Note Voucher Register')
...
<div class="page-header d-print-none">
        <div class="container-xl">
            <div class="card border-0 shadow-sm rounded-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fa);">
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                  ...
                  @canany(['debit_note_voucher.print', 'debit_note_voucher.export'])
                    <div class="dropdown">
                        <button
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            @php
                                $actions = exportActions('debit-note-vouchers', 'debit_note_voucher');
                            @endphp
                ...
                @can('debit_note_voucher.create')
                    <a href="{{ route('debit-note-vouchers.create') }}"
    ...
    <div class="page-body">
        <div class="container-xl">
            <div class="card">
                <form id="debit_note_voucher_register_form" autocomplete="off">
                    <!-- Filters -->
                    <div class="p-3">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card sm-shadow rounded-0 bg-light">
                        <div class="card-body">
                            <div id="debit_note_voucher_register_table">
...
@include('company.pages.debit-note-voucher._view_modal')
@include('company.partials._shortcuts-bar', [
    'barModuleLabel'     => 'Debit Note Voucher List',
    'barModuleShortcuts' => [
...
<script>
    const journalVoucherListUrl = "{{ route('debit-note-vouchers.index') }}";
    const journalVoucherReferenceUrl = "{{ route('debit-note-vouchers.voucher_reference', ['journalVoucher' => '__ID__']) }}";
    const journalVoucherViewUrl = "{{ route('debit-note-vouchers.view', ':id') }}";
    const journalVoucherEditUrl = "{{ route('debit-note-vouchers.edit') }}";
    const journalVoucherIndividualPrintUrl = "{{ route('debit-note-vouchers.print-voucher', '') }}";
    const journalVoucherPrintReceiptUrl = "{{ route('debit-note-vouchers.print-journal', '') }}";
    const journalVoucherPrintUrl = "{{ route('debit-note-vouchers.print') }}";
    const journalVoucherExcelUrl = "{{ route('debit-note-vouchers.export.excel') }}";
    const journalVoucherDeleteUrl = "{{ route('debit-note-vouchers.destroy', ':id') }}";
</script>

@section('script')
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/modules/debit-note-voucher/index.js') }}?v={{ hash_file('md5', public_path('js/modules/debit-note-voucher/index.js')) }}"></script>
@endsection
