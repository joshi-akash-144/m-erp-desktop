                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                @php
                                $actions = exportActions('payment-vouchers', 'payment_voucher');
                                @endphp
                            ...
                            <div class="d-flex flex-wrap justify-content-md-end justify-content-start gap-2">
                                <div class="waves-effect d-flex gap-2">
                                    <!-- @can('payment_voucher.update')
                                        <a href="{{ route('payment-vouchers.edit') }}"
...
<div class="page-body">
    <div class="container-xl">
        <div class="card">
            <form id="payment_voucher_register_form" autocomplete="off">
                <!-- Filters -->
                <div class="p-3">
...
'barModuleLabel' => 'Payment Voucher List',
'barModuleShortcuts' => [
...
<script>
    const paymentVoucherListUrl = "{{ route('payment-vouchers.index') }}";
    const paymentVoucherReferenceUrl = "{{ route('payment-vouchers.voucher_reference', ['paymentVoucher' => '__ID__']) }}";
    const paymentVoucherPrintUrl = "{{ route('payment-vouchers.print') }}";
    const paymentVoucherEditUrl = "{{ route('payment-vouchers.edit') }}";
    const paymentVoucherIndividualPrintUrl = "{{ route('payment-vouchers.print-voucher',':id') }}";
    const paymentVoucherViewUrl = "{{ route('payment-vouchers.view',':id') }}";
    const paymentVoucherExcelUrl = "{{ route('payment-vouchers.export.excel') }}";
    const paymentVoucherAdvicePrintUrl = "{{ route('payment-vouchers.payment_advice_print',':id') }}";
    const paymentVoucherDeleteUrl = "{{ route('payment-vouchers.destroy', ':id') }}";
</script>

@section('script')
<script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
<script src="{{ asset('js/modules/payment-voucher/index.js') }}?v={{ hash_file('md5', public_path('js/modules/payment-voucher/index.js')) }}"></script>
@endsection