...
<head>
    <meta charset="utf-8">
    <title>Receipt Voucher Register - Print</title>
...
<body>
    @php
    $reportTitle = "Receipt Voucher Register";
    ...
    foreach($transactions as $index => $item) {
        ...
        if ($index === $totalItems - 1) {
            // Last item needs 1 extra row for Grand Total
            ...
            <table class="report-table">
                ...
                <tbody>
                    ...
                    @forelse($transactions as $item)
                        ...
                        <tr>
                            <td class="text-start text-nowrap" style="width: 12%;">{{ $item->voucher_date ? format_date($item->voucher_date) : '' }}</td>
                                @if(isset($item->row_type) && $item->row_type == "transaction")
                                    <td class="text-start text-nowrap text-truncate" style="width: 30%;max-width: 0;">
                                        {{ $item->particulars ?? '' }}
                                    </td>
                                    <td class="text-center text-nowrap" style="width: 10%;">{{ $item->voucher_number ?? '' }}</td>
                                    <td class="text-end text-nowrap" style="width: 12%;">{{ $item->debit >0 ? formatIndianNumber($item->debit) : '' }}</td>
                                    <td class="text-end text-nowrap" style="width: 12%;">{{ $item->credit >0 ? formatIndianNumber($item->credit) : '' }}</td>
                                @else
                                    <td class="text-start text-nowrap text-truncate" style="max-width: 0;" colspan="4">
