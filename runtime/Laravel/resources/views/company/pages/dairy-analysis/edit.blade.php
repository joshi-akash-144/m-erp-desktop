...
@section('title', 'Dairy Analysis — Edit')
...
<style>
    ...
    /* ── Select2 override for analysis picker ─────────────── */
...
<div class="page-wrapper" style="max-width: 1700px">
    ...
    <form id="dairy_analysis_form" data-mode="update">
        <input type="hidden" name="uuid" id="uuid" value="{{ uuid() }}">
        @csrf

        {{-- ══════════════════════════════════════════════════
             TOP BAR — title · analysis picker · action buttons
        ══════════════════════════════════════════════════ --}}
            ...
            <div class="card-body p-1 d-flex flex-wrap align-items-center gap-3  border-start border-4 border-dairy-analysis">
                ...
                <div class="d-flex align-items-center gap-2 pe-3 border-end flex-shrink-0" style="width: 400px;">
                    ...
                    <div class="lh-sm d-flex flex-row gap-2">
                        ...
                        <span class="badge badge-pill bg-orange text-white fw-bold text-uppercase shadow-sm px-2 py-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">
            ...
            <div class="da-section da-section-sales shadow-sm">
                ...
                <div class="da-field-grid">
                    ...
                    <div class="da-field-item" style="grid-column: span 1;">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <select id="analysis_select" class="form-select form-select-sm fw-bold da-field-val" style="max-width: 250px; font-size: 0.95rem;">
                                <option value="">-- Select Bill --</option>
                                @foreach($analysisList as $item)
                                    <option value="{{ $item->id }}">
                                        {{ $item->sales_inv_number ?? 'Analysis #' . $item->id }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="card-footer bg-light border-top p-3">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <small class="text-muted d-flex align-items-center gap-2">
                        ...
                        Select a bill above to load data, then update the actual values
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="submit" id="btn_submit"
                                class="btn btn-warning px-4 form-save-btn waves-effect"
...
@section('script')
<script>
    const getAnalysisByIdUrl = "{{ route('dairy-analysis.get_analysis_by_id') }}";
...
<script src="{{ asset('js/modules/dairy-analysis/pds-email-modal.js') }}?v={{ hash_file('md5', public_path('js/modules/dairy-analysis/pds-email-modal.js')) }}"></script>
<script src="{{ asset('js/modules/dairy-analysis/edit.js') }}?v={{ hash_file('md5', public_path('js/modules/dairy-analysis/edit.js')) }}"></script>
@endsection
