...
@section('title', 'Journal Voucher Register')
...
<div class="page-header d-print-none">
        <div class="container-xl">
            <div class="card border-0 shadow-sm rounded-0" style="background: linear-gradient(145deg, #ffffff, #f8f9fa);">
                <div class="card-body p-1 d-flex flex-wrap align-items-center justify-content-between gap-3 border-start border-4 border-primary rounded-0">
                  ...
                  @canany(['journal_voucher.print', 'journal_voucher.export'])
                    <div class="dropdown">
                        <button
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            @php
                                $actions = exportActions('journal-vouchers', 'journal_voucher');
                            @endphp
    ...
    <div class="page-body">
        <div class="container-xl">
            <div class="card">
                <form id="journal_voucher_register_form" autocomplete="off">
                    <!-- Filters -->
                    <div class="p-3">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card sm-shadow rounded-0 bg-light">
                        <div class="card-body">
                            <div id="journal_voucher_register_table">
...
@include('company.pages.journal-voucher._view_modal')
@include('company.partials._shortcuts-bar', [
    'barModuleLabel'     => 'Journal Voucher List',
    'barModuleShortcuts' => [
...
<script>
    const journalVoucherListUrl = "{{ route('journal-vouchers.index') }}";
    const journalVoucherReferenceUrl = "{{ route('journal-vouchers.voucher_reference', ['journalVoucher' => '__ID__']) }}";
    const journalVoucherViewUrl = "{{ route('journal-vouchers.view', ':id') }}";
    const journalVoucherEditUrl = "{{ route('journal-vouchers.edit') }}";
    const journalVoucherIndividualPrintUrl = "{{ route('journal-vouchers.print-voucher', '') }}";
    const journalVoucherPrintReceiptUrl = "{{ route('journal-vouchers.print-journal', '') }}";
    const journalVoucherPrintUrl = "{{ route('journal-vouchers.print') }}";
    const journalVoucherExcelUrl = "{{ route('journal-vouchers.export.excel') }}";
    const journalVoucherDeleteUrl = "{{ route('journal-vouchers.destroy', ':id') }}";
</script>

@section('script')
    <script src="{{ asset('js/utils/icons.js') }}?v={{ hash_file('md5', public_path('js/utils/icons.js')) }}"></script>
    <script src="{{ asset('js/modules/journal-voucher/index.js') }}?v={{ hash_file('md5', public_path('js/modules/journal-voucher/index.js')) }}"></script>
@endsection
