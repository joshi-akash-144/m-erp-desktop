/**
 * Credit Note (Sale Return) — create.js
 *
 * Handles:
 *  - Date pickers with FY validation
 *  - Customer → load sales invoices for Bill No.
 *  - Invoice Type (Sale Type) → populate GST rates
...
$(document).ready(function () {
    ...
    // ── Date pickers ──────────────────────────────────────────────────────────
    new DateInput('#credit_note_date', FINANCIAL_YEAR_START, FINANCIAL_YEAR_END);

    setTimeout(function () {
        $('#credit_note_date').val(currentDate).trigger('change');
    }, 100);

    $('#credit_note_date').on('blur', function () {
    ...
    // ── Edit-mode: auto-load CN data ──────────────────────────────────────────
    if (typeof editCreditNoteId !== 'undefined' && editCreditNoteId) {
        loadEditData(editCreditNoteId);
    }
    setTimeout(() => {
        $('#credit_note_date').focus().select();
    }, 100);
...
function bindSelect2() {
    ['#account_id', '#sales_invoice_id', '#sale_type_id', '#credit_note_id', '.item_id', '.condition_id', '.destination_id']
        .forEach(function (el) {
            $(el).select2({ theme: 'bootstrap-5' });
        });

    $('#account_id').on('change', function () {
        // Reset bill and invoice type when customer changes
        $('#sales_invoice_id').val('').trigger('change.select2');
        $('#sales_invoice_serial').val('');
        $('#sale_type_id').val('').trigger('change');
        window.invoiceSaleTypeMap = {};

        loadSalesInvoicesForCustomer($(this).val());
    });

    // Bill No. change → populate serial hidden + auto-set sale_type_id
    $('#sales_invoice_id').on('change', function () {
        var selected    = $(this).find('option:selected');
        var invoiceId   = $(this).val();
        $('#sales_invoice_serial').val(selected.data('serial') || '');

        var saleTypeId = window.invoiceSaleTypeMap[invoiceId];
        if (saleTypeId) {
            $('#sale_type_id').val(saleTypeId).trigger('change');
        }
    });

    // Invoice Type change → load GST rates
    $('#sale_type_id').on('change', function () {
        loadSaleTypeDetails($(this).val());
    });

    // Credit Note change → reload data in edit mode
...
// ────────────────────────────────────────────────────────────────────────────
// LOAD SALES INVOICES FOR CUSTOMER
// ────────────────────────────────────────────────────────────────────────────
window.invoiceSaleTypeMap = {};

function loadSalesInvoicesForCustomer(accountId) {
    var $select = $('#sales_invoice_id');
    ...
    $.ajax({
        url: salesInvoicesRoute,
        type: 'GET',
        data: { account_id: accountId },
        beforeSend: function () {
            showLoader("Loading Sales Inv Bill");
        },
        success: function (res) {
            if (res.success && res.data) {
                res.data.forEach(function (inv) {
                    let date = formatDateToDMY(inv.invoice_date);
                    $select.append(
                        '<option value="' + inv.id + '" data-serial="'+ inv.invoice_serial + '">'
                        + inv.invoice_serial + ' (' + date + ' | ₹' + inv.net_amount + ')'
                        + '</option>'
                    );
                    if (inv.sale_type_id) {
                        window.invoiceSaleTypeMap[inv.id] = inv.sale_type_id;
                    }
                });
...
// SALE TYPE — GST RATES
// ────────────────────────────────────────────────────────────────────────────
function loadSaleTypeDetails(saleTypeId) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            BillSundry.setSaleTypeDetail(d);
...
function validateForm() {
    $('#credit_note_form').on('submit', function (e) {
        ...
        if (!formData.account_id)                        { showToast('error', 'Please select a customer.');      return; }
        if (!formData.sale_type_id)                      { showToast('error', 'Please select an invoice type.'); return; }
        ...
        var url    = (mode === 'edit') ? cnUpdateUrl + '/' + formData.credit_note_id : cnStoreUrl;
...
function collectFormData() {
    var $form = $('#credit_note_form');
    var data  = {
        uuid:                  $form.find('[name="uuid"]').val(),
        credit_note_id:        $form.find('[name="credit_note_id"]').val(),
        credit_note_date:      formatDateToYMD($('#credit_note_date').val()),
        reference_number:      ($('#reference_number').val() || '').trim(),
        account_id:            $('#account_id').val(),
        sale_type_id:          $('#sale_type_id').val(),
        sales_invoice_id:      $('#sales_invoice_id').val()      || null,
        sales_invoice_serial:  $('#sales_invoice_serial').val()  || null,
        remarks:               $('#remarks').val(),
        items:                 [],
        bill_sundries:         [],
    };
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                Swal.fire({
                    title: 'Success!',
                    html: res.message || 'Credit Note created successfully.',
                    icon: 'success',
...
// ────────────────────────────────────────────────────────────────────────────
function loadEditData(cnId) {
    $.ajax({
        url:  cnEditUrl || (cnUpdateUrl + '/edit'),
        type: 'GET',
        data: { credit_note_id: cnId },
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        success: function (res) {
            if (!res.success || !res.data) { showToast('error', 'Failed to load data.'); return; }
            populateForm(res.data.creditNote);
        },
        error: function () { showToast('error', 'Failed to load Credit Note data.'); }
    });
}

function populateForm(cn) {
    $('#credit_note_date').val(formatDateToDMY(cn.credit_note_date));
    $('#reference_number').val(cn.reference_number || '');
    $('#account_id').val(cn.account_id).trigger('change');

    // Load invoices for the customer then set the value
    loadSalesInvoicesForCustomer(cn.account_id);
    setTimeout(function () {
        $('#sales_invoice_id').val(cn.sales_invoice_id || '').trigger('change.select2');
        $('#sales_invoice_serial').val(cn.sales_invoice_serial || '');
    }, 600);

    $('#sale_type_id').val(cn.sale_type_id).trigger('change');
    $('#remarks').val(cn.remarks || '');

    // Items
    $('#item_table_body').empty();
    itemRowIndex = 0;
    if (cn.details && cn.details.length) {
        cn.details.forEach(function (row) {
    ...
    // Bill sundries
    if (cn.bill_sundries && cn.bill_sundries.length) {
        fillBillSundryData(cn.bill_sundries);
    }
}
