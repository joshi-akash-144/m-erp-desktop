/* Salary Voucher Register – index.js (Tabulator) */

var table;
var FILTER_KEY = 'salary_module_register_filter';
...
function initTabulator() {
    table = new Tabulator('#salary_module_register_table', {
        height: '600px',
        layout: 'fitColumns',
...
var viewItemsTable = null;

function viewVoucher(id) {
    // Select the row visually
    var row = table.getRows().find(r => r.getData().id === id);
    if (row) {
        table.deselectRow();
        row.select();
    }
    ...
    var $modal = $('#viewExpenseModal');
    $modal.modal('show');
    
    $('#view_loading_spinner').removeClass('d-none');
    $('#view_modal_content').addClass('d-none');
    $('#view_voucher_no, #view_date, #view_month, #view_expense_dr, #view_total_amount').text('-');

    // Fetch data
    $.ajax({
        url: salaryModuleDataUrl.replace(':id', id),
        type: 'GET',
        success: function(data) {
            ...
            const renderContent = function() {
                ...
                $('#view_month').text(data.month || rowData.month || '-');
                $('#view_expense_dr').text(rowData.expense_account_name || '-');
                $('#view_total_amount').text(fmt2(data.total_amount));
                ...
                viewItemsTable = new Tabulator("#view_items_table", {
                    ...
                    columns: [
                        { title: "Party Name", field: "account_name", minWidth: 150, headerSort: false },
                        { title: "Vehicle", field: "vehicle_name", minWidth: 150, headerSort: false },
                        { title: "Ref No", field: "bill_no", width: 120, headerSort: false },
...
function printVoucher(id) {
    printReport(salaryModulePrintUrl.replace(':id', id));
}

/* ── Edit: individual voucher ───────────────────────────────────────────────── */

function editVoucher(id) {
    window.location.href = salaryModuleEditUrl.replace(':id', id);
}

/* ── Print: register ─────────────────────────────────────────────────────────── */

function printRegister() {
    printReport(salaryModuleRegisterPrintUrl, currentParams());
}
...
function restoreFilters() {
    try {
        ...
        if (saved.from_date || saved.to_date || saved.filter_voucher || saved.filter_expense_account) {
            table.setData();
        }
    } catch (e) { /* ignore */ }
}
...
function bindSelect2() {
    const selectIdArray = [
        '#filter_voucher, #filter_expense_account'
    ];
