...
function getOldChequeData() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            // $("#cheque_number").val(res.data.cheque_no);
...
function chequePrint() {
  const paymentDate = $("#rtgs_date").val().trim()
    ...
    : null;
  const bankId = $("#bank_id").val() || null;
  const chequeNo = $("#cheque_number").val() || null;
...
function sendToBankEmail() {
  ...
  $.ajax({
    ...
    beforeSend: () => showLoader('Preparing email preview…'),
    success: function (res) {
      hideLoader();
      if (!res.success) {
...
--------------------------------------------------------------*/
let isPaSingleSending = false;
$(document).off('click', '#pa_email_send_btn').on('click', '#pa_email_send_btn', function () {
  if (isPaSingleSending) return;
...
| Bulk send button
--------------------------------------------------------------*/
let isPaBulkSending = false;
$(document).off('click', '#pa_bulk_send_btn').on('click', '#pa_bulk_send_btn', function () {
  if (isPaBulkSending) return;
  
  const voucherIds = JSON.parse($('#pa_bulk_modal_voucher_ids').val() || '[]');

  if (voucherIds.length === 0) return;

  isPaBulkSending = true;
  const $btn = $(this).prop('disabled', true)
    .html('<span class="spinner-border spinner-border-sm me-1"></span> Sending…');
...
$(document).on("input change", "#rtgs_date", function () {
