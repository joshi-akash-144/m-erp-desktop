$(document).ready(async function () {
  ...
  if (formMode === "edit") {
    ...
    await bindTableEvent();

    const urlParams = new URLSearchParams(window.location.search);
    const voucherIdParam = urlParams.get('voucher_id');
    
    if (voucherIdParam) {
        $("#voucher_id").val(voucherIdParam).trigger('change');
    } else {
        setTimeout(() => {
          $("#voucher_id").focus();
        }, 50);
...
async function bindTableEvent() {
  await preloadAccounts();
...
class VoucherTableManager {
  ...
  async handleStore(e) {
    ...
    const existingPayment = window.currentVoucherPayment;
    if (existingPayment) {
        payload.cheque_action   = existingPayment.mode || "voucher_only";
        payload.bank_account_id = existingPayment.bank_id;
        payload.party_account_id = existingPayment.account_id;
        payload.bank_amount     = existingPayment.paid_amount || 0;
        
        if (existingPayment.payment) {
            payload.cheque = {
                cheque_no: existingPayment.payment.cheque_number,
                cheque_date: existingPayment.payment.cheque_date,
            };
        }
    } else {
  ...
  async submitVoucher(payload) {
    ...
    try {
      ...
      const swalOpts = {
        icon: "success",
        title: "Success",
        html: response.message + (!isEdit && actionLabel ? `<br><small class="text-muted">${actionLabel}</small>` : ""),
      ...
      let msg = err?.responseJSON?.message || "Something went wrong. Please try again.";
      if (err?.responseJSON?.errors && Object.keys(err.responseJSON.errors).length > 0) {
      ...
      $btn
        .prop("disabled", false)
        .html(
          isEdit ? '<i class="fa-solid fa-pencil me-1"></i> Update' : '<i class="fa-solid fa-floppy-disk me-1"></i> Save',
        );
    }
  }
...
function getVoucherDetails(voucherId) {
    $.ajax({
        url: editPaymentVoucher,
        beforeSend: function () {
            showLoader("Fetching Payment Voucher Details...");
        },
        method: "GET",
...
function setVoucherDetails(response) {
    const { voucher, payment, is_locked, is_paid, locked_refs, entry_from } = response.data;
    const manager = window.voucherTableManager;
    window.currentVoucherPayment = payment;
    ...
    if (entry_from === "payment_payable") {
        Swal.fire({
            icon: "warning",
            title: "Voucher Locked",
            html: "This voucher cannot be edited because it was generated from Payment Payable.<br><br><span class='text-muted'>Any changes must be made in the original module.</span>",
            confirmButtonColor: "#d9534f",
            confirmButtonText: "OK",
        });
        $("#save_btn").prop("disabled", true).html('<i class="fa-solid fa-lock me-1"></i> Locked');
    } else if (!response.data.is_voucher_only && (is_paid || voucher.is_approved === 1)) {
        Swal.fire({
            icon: "warning",
            title: "Voucher Locked",
            html: "This voucher cannot be edited because it has already been approved or paid.<br><br><span class='text-muted'>To make changes, it must be unapproved/unpaid first.</span>",
            confirmButtonColor: "#d9534f",
            confirmButtonText: "OK",
        });
        $("#save_btn").prop("disabled", true).html('<i class="fa-solid fa-lock me-1"></i> Locked');
        ...
        Swal.fire({
            icon: "warning",
            title: "Voucher Locked",
            html: `This voucher cannot be edited because the following reference(s) have already been used in another voucher:<ul class="text-start mt-2">${refList}</ul>`,
            confirmButtonColor: "#d9534f",
            confirmButtonText: "OK",
        });
        $("#save_btn").prop("disabled", true).html('<i class="fa-solid fa-lock me-1"></i> Locked');
    ...
    $("#voucher_serial").val(voucher.voucher_serial);
    $("#uuid").val(voucher.uuid);
    ...
    details.forEach((txn, index) => {
        ...
        if (accountData) {
            ...
            manager.dataStore.setRowData(index, accountData);
        }
    ...
    details.forEach((txn, index) => {
        ...
        const poRef = txn.refs.find((r) => r.purchase_order_id);
        if (poRef) {
            manager.dataStore.setPurchaseOrder(index, {
                id: poRef.purchase_order_id,
                order_serial: poRef.purchase_order_number || "",
                order_number: poRef.purchase_order_number || "",
            });
        }
    });
