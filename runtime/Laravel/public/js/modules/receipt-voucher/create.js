$(document).ready(function () {
  ...
  if (formMode === "edit") {
    ...
    setTimeout(() => { $("#voucher_id").focus(); }, 50);
  } else {
    $("#voucher_date").focus();
  }
...
function bindTableEvent() {
  preloadAccounts();
  window.voucherTableManager = new VoucherTableManager("#receipt_voucher_table");
}
...
function initSelect2ForAllRows() {
  ...
  $(".account-id").on("select2:open", function () {
    let selectEl = $(this);
    var searchInput = selectEl.data("select2").$dropdown.find(".select2-search__field");
...
class VoucherTableManager {
  constructor(tableSelector) {
    ...
    this.referenceManager = new ReferenceManager(this.dataStore, this.rowManager);
  ...
  initEvents() {
    ...
    this.table.on("focus", ".debit-amount, .credit-amount", (e) => this.autoBalance(e));
    ...
    $("#receipt_voucher_form").on("submit", (e) => this.handleStore(e));
  }
  ...
  async handleStore(e) {
    ...
    const url = isEdit ? `${updateReceiptVoucher}/${voucherId}` : storeReceiptVoucher;
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Success",
        html: `Receipt Voucher <b class='text-primary'>${response.data.voucher_serial}</b> has been ${action} successfully.`,
      ...
      const msg = err?.responseJSON?.errors || err?.responseJSON?.message || "An error occurred. Please try again.";
  ...
  predictDrCr() {
    ...
    return "cr";                                              // both zero → first row defaults CR
  }
  ...
  handleSelect2Open(e) {
    ...
    const debitAmount = $row.find(".debit-amount").val()?.replace(/,/g, "") || 0;
    const creditAmount = $row.find(".credit-amount").val()?.replace(/,/g, "") || 0;
    const totals = this.calculationService.calculateTotals();

    if (totals.isBalanced && totals.debit > 0 && debitAmount == 0 && creditAmount == 0) {
      setTimeout(() => { $("#voucher_narration").focus(); }, 10);
    }
  }
...
class ReferenceManager {
  ...
  open(row) {
    ...
    if (typeof showLoader === "function") showLoader("Loading Reference Form...");

    this.clear();
    const formattedAmount = typeof formatIndianNumber === "function" ? formatIndianNumber(rowAmount) : rowAmount;
    this.refAmount.text(`${formattedAmount} ${transactionType === "DR" ? "Dr" : "Cr"}`);
    this.refAccountName.text(row.find(".account-id option:selected").text() || "");
  ...
  createRow(sno, data = {}, transactionType = "") {
    ...
    row.find(".ref-file-no").val(data.file_no || "");
  ...
  saveReferences(e) {
    ...
    const requiredAmount = transactionType === "DR" ? amounts.debit : amounts.credit;
  ...
  calculateTotals() {
    ...
    const formatted = typeof formatIndianNumber === "function"
  ...
  createPendingRefRow(ref) {
    const refDate = ref.reference_date
      ? typeof formatYMDtoDMY === "function" ? formatYMDtoDMY(ref.reference_date) : ref.reference_date
      : "";
  ...
  setSelectedReferences(refs) {
    ...
    refs.forEach((ref, i) => {
      const newRow = this.createRow(
        i + 1,
        { refId: ref.refId, method: "against_ref", refName: ref.refNo, refAmt: ref.amount, file_no: ref.fileNo },
        ref.transactionType,
      );
...
class VoucherRowManager {
  ...
  getRowIndex(row) { return this.table.find("tbody tr").index(row); }
  getRow(element)  { return $(element).closest("tr"); }
  ...
  getAccountId(row)      { return row.find(".account-id").val(); }
  getTransactionType(row){ return row.find(".dr-cr").val(); }
  ...
  getAllRows() { return this.table.find("tbody tr"); }
}
...
class VoucherCalculationService {
  ...
  calculateTotals() {
    ...
    this.rowManager.getAllRows().each(function () {
      drTotal += parseFloat($(this).find(".debit-amount").val().replace(/,/g, "")) || 0;
      crTotal += parseFloat($(this).find(".credit-amount").val().replace(/,/g, "")) || 0;
    });
...
class VoucherDataStore {
  ...
  setRowData(rowIndex, data) { this.rowData.set(rowIndex, { ...this.rowData.get(rowIndex), ...data }); }
  getRowData(rowIndex)       { return this.rowData.get(rowIndex) || {}; }

  setReferences(rowIndex, references)  { this.references.set(rowIndex, references); }
  getReferences(rowIndex)              { return this.references.get(rowIndex) || []; }
  clearReferences(rowIndex)            { this.references.delete(rowIndex); }

  setLedgerBalance(accountId, balance) { this.ledgerBalances.set(accountId, balance); }
  getLedgerBalance(accountId)          { return this.ledgerBalances.get(accountId); }
