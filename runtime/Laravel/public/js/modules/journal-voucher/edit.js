...
function resetVoucherForm() {
    ...
    if ($("#vehicle_id").length) $("#vehicle_id").val("").trigger("change");
...
function getVoucherDetails(voucherId) {
    $.ajax({
        url: editJournalVoucher,
        beforeSend: function () {
            showLoader("Fetching Journal Voucher Details...");
        },
        method: "GET",
...
function setVoucherDetails(response) {
    ...
    const { voucher, journal, tds_entry, is_locked, locked_refs } = response.data;
    const manager = window.voucherTableManager;

    window.isVoucherLoading = true;

    if (tds_entry) {
        window.existingTdsEntry = tds_entry;
        $('#tds_ref_no').val(tds_entry.reference_no || '');
        $('#tds_party_pan').val(tds_entry.pan_no || '');
    } else {
        window.existingTdsEntry = null;
    }
    ...
    if ($("#vehicle_id").length && journal && journal.vehicle_id) {
        $("#vehicle_id").val(journal.vehicle_id).trigger("change");
    } else if ($("#vehicle_id").length) {
    ...
    // Slight delay to ensure all async blur/change events from row rendering have completed
    setTimeout(() => {
        window.isVoucherLoading = false;
    }, 200);
}
