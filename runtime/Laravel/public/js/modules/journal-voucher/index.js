...
const WIDTH_KEY = "journal_voucher_register_col_widths";
const PAGE_SIZE_KEY = "journal_voucher_page_size";
const PAGE_NUM_KEY  = "journal_voucher_page_num";
...
$(document).ready(function () {
    ...
    table = new Tabulator("#journal_voucher_register_table", {
        height: "550px",
        layout: "fitColumns",
        placeholder: `
    ...
    $(`<style>#journal_voucher_register_table .tabulator-row { cursor: pointer !important; }</style>`).appendTo("head");
    ...
    return [
        ...
        // =======================
        {
            ...
            formatter: (cell) => {
                ...
                if (canView) {
                    actions += `
                        <span class="erp-btn-icon view" data-id="${row.voucher_id}" title="View Journal Voucher">
                            ${viewIcon}
                        </span>`;
                }
                if (canUpdate) {
                    actions += `
                        <a href="${typeof journalVoucherEditUrl !== 'undefined' ? journalVoucherEditUrl : ''}?id=${row.voucher_id}" class="erp-btn-icon edit" title="Edit Journal Voucher">
                            ${editIcon}
                        </a>`;
                }
                if (canPrint) {
                    actions += `
                        <span class="erp-btn-icon print" data-id="${row.voucher_id}" title="Print Journal Voucher">
                            ${printIcon}
                        </span>`;
                }
                if (canDelete) {
                    actions += `
                        <span class="erp-btn-icon delete text-danger" data-id="${row.voucher_id}" title="Delete Journal Voucher">
                            ${deleteIcon}
                        </span>`;
                }
                actions += "</div>";
...
$(document).on("click", ".view", function (event) {
  ...
  $.ajax({
    ...
    type: "GET",
    beforeSend: function () {
      showLoader("Loading Journal Voucher...");
    },
    success: function (response) {
      ...
      $("#journal_voucher_modal").data("voucher-id", voucherId);
      $("#journal_voucher_modal").data(
        "journal-voucher-id",
      ...
      $("#journal_voucher_modal").modal("show");
    },
    error: function (xhr) {
...
$(document).on("click", ".erp-btn-icon.delete", function () {
    const voucherId = $(this).data("id");
    if (!voucherId) return;
    deleteJournalVoucher(voucherId);
});

// Delete / Entry-Reverse from modal
$(document).on("click", "#btn_entry_reverse", function () {
    const voucherId = $("#journal_voucher_modal").data("voucher-id");
    if (!voucherId) return;
    deleteJournalVoucher(voucherId, true);
});

function deleteJournalVoucher(voucherId, fromModal = false) {
    Swal.fire({
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    Swal.fire({
                        icon: "success",
                        title: "Deleted!",
                        text: res.message || "Journal Voucher deleted successfully.",
                        timer: 1800,
...
// Modal Print Action
$(document).on("click", "#btn_print_journal", function (e) {
    e.preventDefault();
    const id = $("#journal_voucher_modal").data("voucher-id");
