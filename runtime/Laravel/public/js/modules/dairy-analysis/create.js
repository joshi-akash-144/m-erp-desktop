$(document).ready(function () {
    ...
    /* =========================================================
     * State — bill IDs captured after Find, used on submit
     * ========================================================= */
    let salesBillId    = null;
    let purchaseBillId = null;
    let pBillPayAmount = 0;  // server-supplied pay amount shown in footer
    ...
    /** Safe float parse — returns fallback instead of NaN */
    function safeFloat(val, fallback = 0) {
        const n = parseFloat(val);
        return isNaN(n) ? fallback : n;
    }

    /** Extract numeric string from a display span (strips ₹, commas, etc.) */
    function spanAmount(selector) {
        return safeFloat($(selector).text().replace(/[^\d.-]/g, ''));
    }

    /** SweetAlert error dialog — returns the promise so callers can chain */
    ...
    // Clear form when bill number field is emptied
    $('#invoice_serial').on('change', function () {
        if (!$(this).val()) clearFormData();
    });

    // Allow pressing Enter inside the bill number field to trigger Find
    $('#invoice_serial').on('keydown', function (e) {
        if (e.which === 13) { e.preventDefault(); $('#btn_find_sales').trigger('click'); }
    });

    $('#btn_find_sales').on('click', function () {
        const invoiceSerial = $('#invoice_serial').val().trim();
        if (!invoiceSerial) {
        ...
        showLoader('Fetching bill details...');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    if (response.is_duplicate && response.dairy_analysis_id > 0) {
                        const editLink = editAnalysisUrl + '?id=' + response.dairy_analysis_id;
                        Swal.fire({
                            icon: 'info',
                            title: 'Already Analysed',
                            html: `This bill has already been analysed.<br><br>
                                   <a href="${editLink}" class="btn btn-sm btn-primary">
                                     <i class="fa-solid fa-pencil me-1"></i> Open in Edit
                                   </a>`,
                    ...
                    alertWarning(response.message || 'No bill found for the entered number.').then(() => {
                        clearFormData();
                        $('#invoice_serial').val('').focus();
                    });
                }
            },
            error: function (xhr) {
            ...
            complete: function () {
                $btn.html('Find').prop('disabled', false);
                hideLoader();
            },
        });
    });
    ...
    function populateFormData(data) {
        ...
        // Switch between Save and Update mode
        if (analysisId) {
            ...
            $form.attr('action', storeAnalysisUrl).data('mode', 'save');
            $submitBtn.html('<i class="fa-solid fa-floppy-disk me-1"></i> Save');
            $printBtn.addClass('d-none').removeAttr('data-record-id');
        ...
        // Sales amount = qty × inclusive rate
        $('#s_bill_amount').text(
        ...
        // Build per-PO-line breakdown from p_details
        let totalQty       = 0;
        ...
        // Show per-PO breakdown strip only when there are multiple detail lines
                ...
                <tr data-range="${item.element_range || 0}" data-element-id="${item.element}">
                    ...
                    <td>
                        <span id="el_rebate_pct_${index}"
                              class="form-control fw-bold border-0 bg-light text-end">0.00</span>
                    </td>
                    <td>
    ...
    function attachTableEvents() {
        ...
        // Recalculate when the user changes an actual value
        $('.actual-input').on('change', function () {
            const index  = $(this).data('index');
            const rawVal = $(this).val();

            if (rawVal === '') {
                // Empty → reset all computed columns for this row
            ...
            // Round to 2dp for slab lookup; keep full precision for diff display
        ...
        // Up / Down / Enter key navigation between actual inputs
        $('.actual-input').on('keydown', function (e) {
            ...
            let next;
            if (e.which === 38)                     next = index - 1; // Up
            else if (e.which === 40 || e.which === 13) next = index + 1; // Down / Enter

            if (next !== undefined) {
                // Format current field before leaving it
    ...
    /**
     * Compute rebate and premium for one parameter row using tiered slabs.
     *
     * Range type stored in data-range on the <tr>:
     *   1 = Standard  — low actual is bad (rebate when actual < guarantee)
     *   2 = Inverse   — high actual is bad (rebate when actual > guarantee)
     *
     * valueToSearch > 0 → rebate situation; < 0 → premium situation.
     */
    function computeRebatePremium(index, actual, guarantee) {
        ...
        // TORN BAGS: rebate = actual value directly (no slab calculation)
        ...
        // Direction guard — same as old code's range check before calling calculateRebate
        ...
        // Sort: nearest-to-guarantee slab first
        ...
        // --- Exact port of old calculateRebate ---
        // Accumulate blocks until the slab the actual value falls into, then
        // subtract the unused portion of that last block.
        const blockWidths  = [];
        const blockRebates = [];
    ...
    function calculateGrandTotals() {
        ...
        // Display the server-supplied pay amount (not recalculated client-side)
    ...
    $('#dairy_analysis_form').on('submit', function (e) {
        ...
        // Warn user when purchase rebate is zero or negative before allowing submit
        if (prebateTotal <= 0) {
            Swal.fire({
                title: 'Warning!',
                text: `Purchase rebate is ${prebateTotal.toFixed(2)}. Are you sure you want to submit?`,
                icon: 'warning',
                showCancelButton: true,
    ...
    function submitForm() {
        ...
        // Collect parameter rows into items array
        const items = [];
        ...
        const formData = {
            ...
            sales_inv_number:       $('#invoice_serial').val(),
        ...
        // Laravel requires POST + _method spoofing for PUT requests
        let method = 'POST';
        if (isUpdate) formData._method = 'PUT';

        $btn.html('<i class="fa fa-spinner fa-spin me-1"></i> Processing...').prop('disabled', true);
        showLoader(isUpdate ? 'Updating...' : 'Saving...');

        $.ajax({
            url:    $form.attr('action') || storeAnalysisUrl,
            method: method,
            data:   formData,
            success: function (response) {
                if (response.success) {
                    Swal.fire({
                        ...
                        text:  response.message || (isUpdate ? 'Analysis updated.' : 'Analysis saved.'),
                        icon:  'success',
                        showCancelButton: true,
                        confirmButtonText: 'OK',
                        cancelButtonText: '<i class="fa-solid fa-envelope me-1"></i> Send Mail',
                        ...
                        if (result.dismiss === Swal.DismissReason.cancel) {
                            const recordId = response.id || $('#btn_print').attr('data-record-id');
                            if (recordId && typeof openDairyAnalysisEmailModal === 'function') {
                                $('#btn_print').attr('data-record-id', recordId);
                                openDairyAnalysisEmailModal([recordId], null);
                                // Reload only after the email modal is fully closed AND no Swal is visible
                                $('#paEmailModal').one('hidden.bs.modal', function () {
                    ...
                    showToast('error', response.message || 'Failed to save. Please try again.');
                }
            },
            error: function (xhr) {
    ...
    function clearFormData() {
        ...
        // Parameter table and totals footer
        $('#parameter_table_body').empty();
        $('#parameter_table_foot').empty();

        // Reset form to Save mode
        $('#dairy_analysis_form').data('mode', 'save').attr('action', storeAnalysisUrl);
        $('#btn_submit').html('<i class="fa-solid fa-floppy-disk me-1"></i> Save');
    ...
    $('#btn_print').on('click', function (e) {
        ...
        if (!recordId) {
            showToast('warning', 'No saved record found to print');
            return;
        }
    ...
    $('#btn_email').on('click', function (e) {
        ...
        if (!recordId) {
            alertWarning('Please select a saved record first.');
            return;
        }
