...
const WIDTH_KEY = "credit_note_voucher_register_col_widths";
const PAGE_SIZE_KEY = "credit_note_voucher_page_size";
const PAGE_NUM_KEY  = "credit_note_voucher_page_num";
...
$(document).ready(function () {
    ...
    table = new Tabulator("#credit_note_voucher_register_table", {
        height: "550px",
        layout: "fitColumns",
        placeholder: `
    ...
    $(`<style>#credit_note_voucher_register_table .tabulator-row { cursor: pointer !important; }</style>`).appendTo("head");
    ...
    return [
        ...
        // =======================
        {
            ...
            formatter: (cell) => {
                ...
                if (canView) {
                    actions += `
                        <span class="erp-btn-icon view" data-id="${row.voucher_id}" title="View Credit Note Voucher">
                            ${viewIcon}
                        </span>`;
                }
                if (canUpdate) {
                    actions += `
                        <a href="${typeof journalVoucherEditUrl !== 'undefined' ? journalVoucherEditUrl : ''}?id=${row.voucher_id}" class="erp-btn-icon edit" title="Edit Credit Note Voucher">
                            ${editIcon}
                        </a>`;
                }
                if (canPrint) {
                    actions += `
                        <span class="erp-btn-icon print" data-id="${row.voucher_id}" title="Print Credit Note Voucher">
                            ${printIcon}
                        </span>`;
                }
                if (canDelete) {
                    actions += `
                        <span class="erp-btn-icon delete text-danger" data-id="${row.voucher_id}" title="Delete Credit Note Voucher">
                            ${deleteIcon}
                        </span>`;
                }
                actions += "</div>";
...
$(document).on("click", ".view", function (event) {
  ...
  $.ajax({
    ...
    type: "GET",
    beforeSend: function () {
      showLoader("Loading Credit Note Voucher...");
    },
    success: function (response) {
      ...
      $("#credit_note_voucher_modal").data("voucher-id", voucherId);
      $("#credit_note_voucher_modal").data(
        "credit-note-voucher-id",
      ...
      $("#credit_note_voucher_modal").modal("show");
    },
    error: function (xhr) {
...
$(document).on("click", ".erp-btn-icon.delete", function () {
    const voucherId = $(this).data("id");
    if (!voucherId) return;
    deleteCreditNoteVoucher(voucherId);
});

// Delete / Entry-Reverse from modal
$(document).on("click", "#btn_entry_reverse", function () {
    const voucherId = $("#credit_note_voucher_modal").data("voucher-id");
    if (!voucherId) return;
    deleteCreditNoteVoucher(voucherId, true);
});

function deleteCreditNoteVoucher(voucherId, fromModal = false) {
    Swal.fire({
        title: "Delete Credit Note Voucher?",
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    Swal.fire({
                        icon: "success",
                        title: "Deleted!",
                        text: res.message || "Credit Note Voucher deleted successfully.",
                        timer: 1800,
...
$(document).on("click", "#btn_print_credit_note", function (e) {
    e.preventDefault();
    const id = $("#credit_note_voucher_modal").data("voucher-id");
