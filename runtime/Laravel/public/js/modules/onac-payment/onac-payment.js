$(document).ready(function () {
  $("#payment_payable_form").on("submit", function (e) {
    ...
    // Check if we are on the ONAC payment page by looking for the modal
    if ($('#onac_payment_list_modal').length > 0) {
      openAdvanceListModal();
    } else {
      getReferenceDetail();
    }
  });
...
// --- ONAC Payment Modal Logic ---
function openAdvanceListModal() {
  let accountId = $("#account_id").val();
  
  if (!accountId) {
      Swal.fire({
          title: "Warning",
          html: `Please Select Account First!`,
          icon: "warning",
          confirmButtonText: "OK",
      });
      return;
  ...
  $.ajax({
    url: getAdvanceListUrl,
    method: "GET",
    data: { account_id: accountId },
    beforeSend: function () {
      showLoader("Loading on account records...");
      $('#onac_advance_table_body').html($('#onac_loader_row').prop('outerHTML'));
      $('#onac_loader_row').removeClass('d-none');
      $('#onac_advance_table_foot').addClass('d-none');
      $('#onac_payment_list_modal').modal('show');
    },
    success: function (response) {
      if (!response.data || response.data.length === 0) {
        $('#onac_advance_table_body').html('<tr><td colspan="10" class="py-4 text-center text-muted">No Advance Purchase Orders Found</td></tr>');
      } else {
        let html = '';
        response.data.forEach(item => {
            let recPercentage = item.qty > 0 ? ((item.rec_qty / item.qty) * 100).toFixed(2) : '0.00';
            let badgeClass = 'bg-secondary text-white';
            if (recPercentage > 0 && recPercentage < 100) {
                badgeClass = 'bg-warning-lt text-warning fw-bold border border-warning';
            } else if (recPercentage >= 100) {
                badgeClass = 'bg-success-lt text-success fw-bold border border-success';
            }
            
            html += `<tr>`;
            html += `<td class="text-center"><input type="checkbox" class="form-check-input onac-advance-checkbox" value="${item.id}" data-amount="${item.amount}"></td>`;
            html += `<td class="fw-bold">${item.po_serial}</td>`;
            html += `<td class="fw-bold">${item.account_name}</td>`;
            html += `<td>${item.po_date}</td>`;
            html += `<td class="text-end fw-bold">${formatIndianNumber(item.amount)}</td>`;
            html += `<td class="text-center">${item.direction}</td>`;
            html += `<td class="text-end">${item.qty}</td>`;
            html += `<td class="text-end">${item.rec_qty}</td>`;
            html += `<td class="text-end">${item.rem_qty}</td>`;
            html += `<td class="text-center"><span class="badge ${badgeClass} shadow-sm" style="font-size: 0.85rem; padding: 0.35em 0.6em;">${recPercentage}%</span></td>`;
            html += `</tr>`;
        });
        $('#onac_advance_table_body').html(html);
      }
      $('#onac_advance_total').text(formatIndianNumber(response.totalAmount));
      $('#onac_advance_amount').text('0.00');
      $('#onac_advance_table_foot').removeClass('d-none');
    },
    error: function () {
      showToast('error', 'Error loading advance records');
      $('#onac_payment_list_modal').modal('hide');
    },
    complete: function () {
      hideLoader();
    }
...
// Select All checkbox logic for ONAC modal
$(document).on('change', '#onac_select_all', function() {
    let checked = $(this).is(':checked');
    $('.onac-advance-checkbox').prop('checked', checked);
    if (checked) {
        $('.onac-advance-checkbox').closest('tr').addClass('table-secondary');
    } else {
        $('.onac-advance-checkbox').closest('tr').removeClass('table-secondary');
    }
    calculateOnacSelectedTotal();
});

// Individual checkbox logic for ONAC modal
$(document).on('change', '.onac-advance-checkbox', function() {
    let total = $('.onac-advance-checkbox').length;
    let checked = $('.onac-advance-checkbox:checked').length;
    $('#onac_select_all').prop('checked', total > 0 && total === checked);
...
function calculateOnacSelectedTotal() {
    let total = 0;
    $('.onac-advance-checkbox:checked').each(function() {
        total += parseFloat($(this).data('amount')) || 0;
    });
    $('#onac_advance_amount').text(formatIndianNumber(total.toFixed(2)));
}



$(document).on('click', '#btn_onac_submit', function() {
    let selectedIds = [];
    $('.onac-advance-checkbox:checked').each(function() {
    ...
    window.selectedAdvanceIds = selectedIds;
    getReferenceDetail();
});

$(document).on('hidden.bs.modal', '#onac_payment_list_modal', function () {
    $('#onac_search').val('');
    $('#onac_select_all').prop('checked', false);
    $('#onac_advance_table_body tr').removeClass('d-none');
    $('#onac_advance_table_body').html('<tr id="onac_no_data_row" class="text-center"><td colspan="10" class="py-4 text-muted">Click "Get On Account Records" to load.</td></tr>');
    $('#onac_advance_table_foot').addClass('d-none');
    $('#onac_advance_amount').text('0.00');
});

$(document).on('keyup', '#onac_search', function () {
    let value = $(this).val().toLowerCase();
    $("#onac_advance_table_body tr").filter(function () {
        // Skip loader and no data row
        if ($(this).attr('id') === 'onac_loader_row' || $(this).attr('id') === 'onac_no_data_row') return;
...
function getReferenceDetail() {
  ...
  let payload = {
    ...
    reference_ids: window.selectedAdvanceIds || []
  };

  // Clear selected advance ids after use
  window.selectedAdvanceIds = [];
