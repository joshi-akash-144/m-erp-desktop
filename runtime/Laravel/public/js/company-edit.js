$(document).ready(function () {
    ...
    // ===== Date Input =====
    if ($('#financial_year_start').length && typeof FINANCIAL_YEAR_START !== 'undefined') {
...
function submitFormAjax(form) {        
    ...
    // Strip mobile number formatting to just 10 digits before sending
    // var mobileValue = formData.get('mobile_number');
    // if (mobileValue) {
    //     var digits = mobileValue.replace(/\D/g, '');
    //     if (digits.length > 10 && digits.startsWith('91')) {
    //         digits = digits.slice(-10);
    //     }
    //     formData.set('mobile_number', digits);
    // }
