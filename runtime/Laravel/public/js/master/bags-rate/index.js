...
function bindModalEvents() {
  $("#bags_rate_modal").on("shown.bs.modal", function () {
    $('#bags_type').focus();
  });

  // Remove focus right before the modal hides to prevent aria-hidden console warnings
  $("#bags_rate_modal").on('hide.bs.modal', function () {
...
function applyFilter() {  
  ...
  localStorage.setItem("bags_rate_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("bags_rate_filter");
...
$(function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("bags_rate_filter");
  if (savedFilter) {
    try {
  ...
  // -----------------------------------
  table = new Tabulator("#bags_rate_table", {
    height: "calc(100vh - 280px)", // dynamic height that fits most ERP screens
    layout: "fitColumns",
    placeholder: `
  ...
  if (typeof registerTable === "function") {
    registerTable("bags_rate_table", table);
  }
  ...
  $(document).on("click", ".js-load-modal", function (event) {
    ...
    $.ajax({
      url: bagsRateCreateUrl,
      beforeSend: function () {
        showLoader("Loading Bags Rate Form...");
      },
      success: function (response) {       
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        $("#bags_rate_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    const url = bagsRateEditUrl.replace(":id", driverId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Bags Rate Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#bags_rate_form").attr("data-method", "PUT");
        }, 200);
        $("#bags_rate_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const driverId = $(this).data("id");
    const url = bagsRateViewUrl.replace(":id", driverId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Bags rate Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
  ...
  $(document).on("click", ".delete", function (event) {
    const driverId = $(this).data("id");
    const url = bagsRateDeleteUrl.replace(":id", driverId);
