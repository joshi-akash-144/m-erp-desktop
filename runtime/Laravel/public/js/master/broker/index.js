...
function getTableColumns(savedColWidths = {}) {
  return [
    ...
    // =======================
    {
      title: "Status",
      field: "status",
      width: savedColWidths.status || 100,
      hozAlign: "center",
      headerHozAlign: "center",
      headerSort: false,
      resizable: true,
    ...
    // =======================
    {
      title: "Created By",
      field: "creator.name",
      width: savedColWidths.created_by || 200,
      hozAlign: "center",
      headerHozAlign: "center",
      headerSort: false,
      resizable: true,
    ...
    // =======================
    {
      title: "Updated By",
      field: "updater.name",
      width: savedColWidths.updated_by || 200,
      hozAlign: "center",
      headerHozAlign: "center",
      headerSort: false,
      resizable: true,
...
function bindModalEvents() {
  $("#broker_modal").on("shown.bs.modal", function () {
    $('#name').focus();

    const select2Array = ['country', 'state'];

    for (let index = 0; index < select2Array.length; index++) {
      const element = select2Array[index];  
      $(`#${element}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#broker_modal"),
      });
    }
  ...
  $("#broker_modal").on('hide.bs.modal', function () {
...
function applyFilter() {
  ...
  localStorage.setItem("broker_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("broker_filter");
...
$(function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("broker_filter");
  if (savedFilter) {
    try {
      currentFilter = JSON.parse(savedFilter);
      if (currentFilter.filter_broker_id) {
        $elementEl.val(currentFilter.filter_broker_id).trigger("change");
      }
  ...
  // -----------------------------------
  table = new Tabulator("#broker_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("broker_table", table);
  }
  ...
  // Get Modal for create new Broker
  $(document).on("click", ".js-load-modal", function (event) {
    ...
    $.ajax({
      url: brokerCreateUrl,
      beforeSend: function () {
        showLoader("Loading Broker Form...");
      },
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    const url = brokerEditUrl.replace(":id", brokerId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Broker Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#broker_form").attr("data-method", "PUT");
        }, 200);
        $("#broker_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const brokerId = $(this).data("id");
    const url = brokerViewUrl.replace(":id", brokerId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Broker Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
  ...
  $(document).on("click", ".delete", function (event) {
    const brokerId = $(this).data("id");
    const url = brokerDeleteUrl.replace(":id", brokerId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Broker...");
          },
          success: function (response) {
            showToast("success", response.message || "Broker deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
