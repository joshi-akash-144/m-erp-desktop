...
function bindModalEvents() {
  $("#sale_type_modal").on("shown.bs.modal", function () {
    $('#name').focus();
    ...
    for (let index = 0; index < select2Array.length; index++) {
      ...
      $(`#${saleType}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#sale_type_modal"),
      });
    }
...
function applyFilter() {
  const groupEl = document.getElementById("sale_type_id");
  ...
  localStorage.setItem("sale_type_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  if(groupEl) $("#sale_type_id").val("").trigger("change");
  if (searchEl) searchEl.value = "";

  currentFilter = {};
  localStorage.removeItem("sale_type_filter");
...
$(function () {
  ...
  // Initialize Select2
  // -----------------------------------
  $("#sale_type_id").select2({
    placeholder: "Select Sale Type",
    allowClear: true,
    theme: "bootstrap-5",
    width: "100%",
  });
  const $elementEl = $("#sale_type_id");
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("sale_type_filter");
  if (savedFilter) {
    try {
  ...
  dynamicFilter("#search, #sale_type_id");


  // -----------------------------------
  // Create Tabulator Table
  // -----------------------------------
  table = new Tabulator("#sale_type_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("sale_type_table", table);
  }
  ...
  // Get Modal for create new Sale Type
  $(document).on("click", ".js-load-modal", function (event) {
    event.preventDefault();

    $.ajax({
      url: saleTypeCreateUrl,
      beforeSend: function () {
        showLoader("Loading Sale Type Form...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        $("#sale_type_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    const saleTypeId = $(this).data("id");
    const url = saleTypeEditUrl.replace(":id", saleTypeId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Sale Type Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#sale_type_form").attr("data-method", "PUT");
        }, 200);
        $("#sale_type_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const saleTypeId = $(this).data("id");
    const url = saleTypeViewUrl.replace(":id", saleTypeId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading sale Type Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
  ...
  $(document).on("click", ".delete", function (event) {
    const saleTypeId = $(this).data("id");
    const url = saleTypeDeleteUrl.replace(":id", saleTypeId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Sale Type...");
          },
          success: function (response) {
            showToast("success", response.message || "Sale Type deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
