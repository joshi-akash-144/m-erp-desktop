...
function bindModalEvents() {
  $("#element_modal").on("shown.bs.modal", function () {
    $('#name').focus();

    const select2Array = ['range'];

    for (let index = 0; index < select2Array.length; index++) {
      const element = select2Array[index];  
      $(`#${element}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#element_modal"),
      });
    }
  ...
  $("#element_modal").on('hide.bs.modal', function () {
...
function applyFilter() {
  const groupEl = document.getElementById("element_id");
  ...
  currentFilter = {};
  if (groupVal) currentFilter.filter_element_id = groupVal;
  if (searchVal) currentFilter.search = searchVal;

  // Save filter settings locally
  localStorage.setItem("element_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  if(groupEl) $("#element_id").val("").trigger("change");
  if (searchEl) searchEl.value = "";

  currentFilter = {};
  localStorage.removeItem("element_filter");
...
$(function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("element_filter");
  if (savedFilter) {
    try {
      currentFilter = JSON.parse(savedFilter);
      if (currentFilter.filter_element_id) {
        $elementEl.val(currentFilter.filter_element_id).trigger("change");
      }
  ...
  dynamicFilter("#search, #element_id");

  // -----------------------------------
  // Create Tabulator Table
  // -----------------------------------
  table = new Tabulator("#element_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("element_table", table);
  }
  ...
  // Get Modal for create new Element
  $(document).on("click", ".js-load-modal", function (event) {
    event.preventDefault();

    $.ajax({
      url: elementCreateUrl,
      beforeSend: function () {
        showLoader("Loading Element Form...");
      },
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    const url = elementEditUrl.replace(":id", elementId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Element Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#element_form").attr("data-method", "PUT");
        }, 200);
        $("#element_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const elementId = $(this).data("id");
    const url = elementViewUrl.replace(":id", elementId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Element Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
  ...
  $(document).on("click", ".delete", function (event) {
    const elementId = $(this).data("id");
    const url = elementDeleteUrl.replace(":id", elementId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Element...");
          },
          success: function (response) {
            showToast("success", response.message || "Element deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
