(function () {
    ...
    $(document).ready(function () {
        // validation
        validator = new JustValidate("#element_form", {
        ...
        validator
            .addField("#name", [
                { rule: "required", errorMessage: "Element name is required" }
            ])
            .addField("#print_name", [
                { rule: "required", errorMessage: "Print name is required" }
            ])
            .addField("#range", [
                { rule: "required", errorMessage: "Range is required" }
            ])
            .onSuccess((event) => {
                event.preventDefault();
                submitFormAjax(document.getElementById("element_form"));
            });
    });
    ...
    function submitFormAjax(form) {
        ...
        const $form = $(form);
        const $btn = $("#element_modal .form-save-btn");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (success) {
                    showToast("success", message || (isEdit ? "Element updated successfully!" : "Element saved successfully!"));
                    if (typeof refreshTable === "function") {
                        refreshTable("element_table");
            ...
            complete: function () {
                ...
                if ($btn.length) {
                    $btn.prop("disabled", false).html(`
                        ...
                        ${isEdit ? "Update Element" : "Save Element"}
                    `);
                }
