...
function bindModalEvents() {
  $("#destination_modal").on("shown.bs.modal", function () {
    $('#name').focus();

    const select2Array = ['account_group_id','is_billwise','country','state','sale_commission_rate','sale_unit_id','purchase_commission_rate','purchase_unit_id'];

    for (let index = 0; index < select2Array.length; index++) {
      const destination = select2Array[index];  
      $(`#${destination}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#destination_modal"),
      });
    }
  ...
  $("#destination_modal").on('hide.bs.modal', function () {
...
function applyFilter() {
  ...
  localStorage.setItem("destination_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("destination_filter");
...
$(function () {
  ...
  const $elementEl = $("#destination_id");
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("destination_filter");
  if (savedFilter) {
    try {
      currentFilter = JSON.parse(savedFilter);
      if (currentFilter.filter_group_id) {
        $elementEl.val(currentFilter.filter_group_id).trigger("change");
      }
  ...
  // -----------------------------------
  table = new Tabulator("#destination_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("destination_table", table);
  }
  ...
  // Get Modal for create new Destination
  $(document).on("click", ".js-load-modal", function (event) {
    event.preventDefault();

    $.ajax({
      url: destinationCreateUrl,
      beforeSend: function () {
        showLoader("Loading Destination Form...");
      },
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    const url = destinationEditUrl.replace(":id", destinationId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Destination Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#destination_form").attr("data-method", "PUT");
        }, 200);
        $("#destination_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const destinationId = $(this).data("id");
    const url = destinationViewUrl.replace(":id", destinationId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Destination Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        ...
        const $modalEl = $("#destination_modal");
  ...
  $(document).on("click", ".delete", function (event) {
    const destinationId = $(this).data("id");
    const url = destinationDeleteUrl.replace(":id", destinationId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Destination...");
          },
          success: function (response) {
            showToast("success", response.message || "Destination deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
