    ...
    },
    {
      ...
      formatter: (cell) => {
        ...
        if (canView) {
          actions += `
            <span 
              data-module="masters/expense-type/modal"
              data-init="openExpenseTypeViewModal"
        ...
        if (canEdit) {
          actions += `
            <span 
              data-module="masters/expense-type/modal"
              data-init="openExpenseTypeEditModal"
...
function bindModalEvents() {
  ...
  $("#expense_type_modal").on('hide.bs.modal', function () {
...
$(document).ready(function () {
  ...
  $(".js-load-modal").on("click", function (event) {   
    $.ajax({
      url: expenseTypeCreateUrl,
      beforeSend: function () {
        showLoader("Loading Expense Type Form...");
      },
  ...
  table = new Tabulator("#expense_type_table", {  
  ...
  $(document).on("click", ".view", function (event) {
    ...
    const url = expenseTypeViewUrl.replace(":id", expenseTypeId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Expense Type Form...");
  ...
  $(document).on("click", ".edit", function (event) {
    const expenseTypeId = $(this).data("id");
    const url = expenseTypeEditUrl.replace(":id", expenseTypeId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Expense Type Form...");
  ...
  $(document).on("click", ".delete", function (event) {
    const expenseTypeId = $(this).data("id");
    const url = expenseTypeDeleteUrl.replace(":id", expenseTypeId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Expense Type...");
          },
          success: function (response) {
            showToast("success", response.message || "Expense Type deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const savedFilter = localStorage.getItem("expense_type_filter");
  if (savedFilter) {
    currentFilter = JSON.parse(savedFilter);
    if (currentFilter.expense_type_id) {
      $('#expense_type_id').val(currentFilter.expense_type_id).trigger('change');
    }
    if (currentFilter.search && searchEl) {
      searchEl.value = currentFilter.search;
    }
  }    
  dynamicFilter("#search, #expense_type_id");

});

function applyFilter() {
  const groupEl = document.getElementById("expense_type_id");
  const searchEl = document.getElementById("search");
  ...
  currentFilter = {};
  if (groupVal) currentFilter.expense_type_id = groupVal;
  if (searchVal) currentFilter.search = searchVal;

  localStorage.setItem("expense_type_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("expense_type_filter");
