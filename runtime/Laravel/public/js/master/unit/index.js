...
function getTableColumns(savedColWidths) {
  return [
    ...
    // =======================
    {
      title: "Print Name",
      field: "print_name",
      width: savedColWidths.print_name || 200,
      headerSort: false,
      resizable: true,
      formatter: (cell) => {
        const val = cell.getValue();
...
function bindModalEvents() {
  ...
  $("#unit_modal").on('hide.bs.modal', function () {
...
function applyFilter() {
  ...
  currentFilter = {};
  if (unitVal) currentFilter.filter_unit_id = unitVal;
  if (searchVal) currentFilter.search = searchVal;
  // Save filter settings locally
  localStorage.setItem("unit_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  // if (unitEl?.tomselect) unitEl.tomselect.setValue("", false);
  if(unitEl) $("#unit_id").val("").trigger("change"); // SELECT2 RESET
  if (searchEl) searchEl.value = "";

  currentFilter = {};
  localStorage.removeItem("unit_filter");
...
function handleColumnResize(column) {
  ...
  localStorage.setItem("unit_col_widths", JSON.stringify(widths));
}
...
$(function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("unit_filter");
  if (savedFilter) {
    try {
      currentFilter = JSON.parse(savedFilter);
      if (currentFilter.filter_unit_id) {
        $unitEl.val(currentFilter.filter_unit_id).trigger("change");
      }
  ...
  dynamicFilter("#search, #unit_id");

  // -----------------------------------
  // Load Saved Column Widths
  // -----------------------------------
  const savedWidths = localStorage.getItem("unit_col_widths");
  if (savedWidths) {
    try {
  ...
  // -----------------------------------
  table = new Tabulator("#unit_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("unit_table", table);
  }
  ...
  // Get Modal for create new Item Group
  $(document).on("click", ".js-load-modal", function (event) {
    event.preventDefault();
    $.ajax({
      url: createRouteUrl,
      beforeSend: function () {
        showLoader("Loading Unit Form...");
      },
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    const url = editRouteUrl.replace(":id", unitId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Unit Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#unit_form").attr("data-method", "PUT");
        }, 200);
        $("#unit_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const unitId = $(this).data("id");
    const url = viewRouteUrl.replace(":id", unitId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Unit Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
  ...
  $(document).on("click", ".delete", function (event) {
    const unitId = $(this).data("id");
    const url = deleteRouteUrl.replace(":id", unitId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Unit...");
          },
          success: function (response) {
            showToast("success", response.message || "Unit deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
