(function () {
    ...
    // validation
    validator = new JustValidate("#godown_form", {
    ...
    validator
        ...
        .addField("#godown_name", [
            ...
            { rule: "maxLength", value: 100, errorMessage: "Godown Name cannot exceed 100 characters" }
        ])
        .onSuccess((event) => {
            event.preventDefault();
            submitFormAjax(document.getElementById("godown_form"));
        });
    ...
    function submitFormAjax(form) {
        ...
        const $form = $(form);
        const $btn = $("#godown_modal .form-save-btn");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (success) {
                    showToast("success", message || (isEdit ? "Godown updated successfully!" : "Godown saved successfully!"));
                    if (typeof table !== "undefined") table.setData(); // Using generic table pointer
                    $('#godown_modal').modal('hide');
                } else {
            ...
            complete: function () {
                if ($btn.length) {
                    $btn.prop("disabled", false).html(`
                        ...
                        ${isEdit ? "Update Godown" : "Save Godown"}
                    `);
                }
                isSubmitting = false;
            }
