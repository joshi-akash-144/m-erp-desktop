...
function getTableColumns(savedColWidths) {
  return [
    ...
    // =======================
    {
      title: "Type",
      field: "type",
      width: savedColWidths.parent_id || 100,
      headerSort: false,
      resizable: true,
      formatter: (cell) => {
        const val = cell.getValue();
    ...
    // =======================
    {
      ...
      formatter: (cell) => {
        ...
        if (canView) {
          actions += `
            <span 
              data-module="masters/tax-category/modal"
              data-init="openTaxCategoryViewModal"
        ...
        if (canEdit) {
          actions += `
            <span 
              data-module="masters/tax-category/modal"
              data-init="openTaxCategoryEditModal"
...
function bindModalEvents() {
  $("#tax_category_modal").on("shown.bs.modal", function () {
    $('#name').focus();

    const select2Array  = ['type', 'zero_tax_type'];

    for (let index = 0; index < select2Array.length; index++) {
      const element = select2Array[index];
      $(`#${element}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#tax_category_modal"),
      });
    }
...
$(document).ready(function () {
  ...
  $('#tax_category_id').select2({
      theme: 'bootstrap-5',
      placeholder: 'Select an option',      
      width: '100%',                 
  });

    // Get Modal for create new tax category
  $(".js-load-modal").on("click", function (event) {   

    $.ajax({
      url: taxCategoryCreateUrl,
      beforeSend: function () {
        showLoader("Loading Tax Category Form...");
      },
      success: function (response) {       
        $("#global_modal_container").html(response.data.html);                
        bindModalEvents();
        $("#tax_category_modal").modal("show");
      },
      error: function (xhr) {
  ...
  // Account List
  table = new Tabulator("#tax_category_table", {  
    ...
    paginationSize: parseInt(localStorage.getItem("tax_category_page_size")) || 50,
    paginationInitialPage: parseInt(localStorage.getItem("tax_category_page_num")) || 1,
  ...
  table.on("pageLoaded", function(pageNo) {
    localStorage.setItem("tax_category_page_num", pageNo);
    localStorage.setItem("tax_category_page_size", table.getPageSize());
  });


    // View tax category
  $(document).on("click", ".view", function (event) {
    const taxCategoryId = $(this).data("id");
    const url = taxCategoryViewUrl.replace(":id", taxCategoryId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Tax category Form...");
  ...
  $(document).on("click", ".edit", function (event) {
    const taxCategoryId = $(this).data("id");
    const url = taxCategoryEditUrl.replace(":id", taxCategoryId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Account Group Form...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        table.replaceData();
  ...
  $(document).on("click", ".delete", function (event) {
    const taxCategoryId = $(this).data("id");
    const url = taxCategoryDeleteUrl.replace(":id", taxCategoryId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          success: function (response) {
            showToast("success", response.message || "Tax Categories deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("tax_category_filter");
  if (savedFilter) {
    currentFilter = JSON.parse(savedFilter);
    if (currentFilter.filter_group_id) {
      $('#tax_category_id').val(currentFilter.filter_group_id).trigger('change');
    }
  ...
  dynamicFilter("#search, #tax_category_id"); 
});
...
function applyFilter() {
  const groupEl = document.getElementById("tax_category_id");
  const searchEl = document.getElementById("search");
  ...
  localStorage.setItem("tax_category_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  $('#tax_category_id').val("").trigger("change");
  if (searchEl) searchEl.value = "";

  currentFilter = {};
  localStorage.removeItem("tax_category_filter");
