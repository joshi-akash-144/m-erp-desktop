(function () {
    ...
    $(document).ready(function () {
        const formEl = document.getElementById("godown_unit_location_form");
        ...
        // validation
        validator = new JustValidate("#godown_unit_location_form", {
            validateBeforeSubmitting: true,
            focusInvalidField: true,
        });

        validator
            .addField("#destination_id", [
                { rule: "required", errorMessage: "Destination is required" }
            ])
        ...
        $("#godown_name").on("keydown", function (e) {
            if (e.which === 13) {
                e.preventDefault();
                $("#godown_remark").focus();
            }
        ...
        $("#godown_remark").on("keydown", function (e) {
            if (e.which === 13) {
                e.preventDefault();
                $(".form-save-btn").focus();
            }
    ...
    function submitFormAjax(form) {
        ...
        const $form = $(form);
        const $btn = $("#godown_unit_location_modal .form-save-btn");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    if (typeof godownUnitLocationTable !== "undefined") {
                        godownUnitLocationTable.replaceData();
                    }
                    $('#godown_unit_location_modal').modal('hide');
                } else {
