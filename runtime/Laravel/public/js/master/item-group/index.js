let table; // Reference to Tabulator table
let currentFilter = {}; // Holds current filter state
let currentPermissions = {}; // Holds current user permissions
let savedColWidths = {}; // Holds saved column widths
...
function getTableColumns(savedColWidths = {}) {
  return [
    ...
    // =======================
    {
      ...
      resizable: true,
      formatter: (cell) => {
        const val = cell.getRow().getData().updater?.name || null;
        return val
          ? `<span class="badge bg-danger-lt ">${val}</span>`
...
function bindModalEvents() {
  $("#item_group_modal").on("shown.bs.modal", function () {
    $('#name').focus();
  });

  // Remove focus right before the modal hides to prevent aria-hidden console warnings
  $("#item_group_modal").on('hide.bs.modal', function () {
...
function applyFilter() {
  ...
  localStorage.setItem("item_group_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("item_group_filter");
...
function handleColumnResize(column) {
  ...
  localStorage.setItem("item_group_col_widths", JSON.stringify(widths));
}
...
$(function () {
  ...
  // -----------------------------------
  const savedWidths = localStorage.getItem("item_group_col_widths");
  if (savedWidths) {
    try {
  ...
  // -----------------------------------
  table = new Tabulator("#item_group_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("item_group_table", table);
  }
  ...
  $(document).on("click", ".js-load-modal", function (event) {
    ...
    $.ajax({
      url: createRouteUrl,
      beforeSend: function () {
        showLoader("Loading Item Group Form...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        $("#item_group_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#item_group_form").attr("data-method", "PUT");
        }, 200);
        $("#item_group_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    ...
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Item Group Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        ...
        const $modalEl = $("#item_group_modal");
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting Item Group...");
          },
          success: function (response) {
            showToast("success", response.message || "Item Group deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            showToast("error", 'Oops! Something went wrong. Try again later.');
