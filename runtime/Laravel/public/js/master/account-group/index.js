...
function getTableColumns(savedColWidths) {
  return [
    ...
    // =======================
    // Account Group Name (Tree Column)
    // =======================
    {
      title: "Account Group Name",
      field: "name",
      width: savedColWidths.name || 500,
      headerSort: true,
      sorter: "string",
      resizable: true,
      // this enables the expand/collapse tree icon in this column
      formatter: "tree",
    },
    ...
    // =======================
    {
      ...
      formatter: (cell) => {
        ...
        if (canView) {
          actions += `
            <span 
              data-module="masters/account-group/modal"
              data-init="openAccountGroupViewModal"
        ...
        if (canEdit) {
          actions += `
            <span 
              data-module="masters/account-group/modal"
              data-init="openAccountGroupEditModal"
...
function bindModalEvents() {
  $("#account_group_modal").on("shown.bs.modal", function () {
    $('#name').focus();
    ...
    for (let index = 0; index < select2Array.length; index++) {
      ...
      $(`#${element}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#account_group_modal"),
      });
    }
...
$(document).ready(function () {
  ...
  // Get Modal for create new account group
  $(".js-load-modal").on("click", function (event) {
    $.ajax({
      url: accountGroupCreateUrl,
      beforeSend: function () {
      ...
      success: function (response) {
        ...
        bindModalEvents();
        $("#account_group_modal").modal("show");
      },
      error: function (xhr) {
  ...
  // Account List
  table = new Tabulator("#account_group_table", {
    dataTree: true,
    ...
    paginationSize: parseInt(localStorage.getItem("account_group_page_size")) || 50,
    paginationInitialPage: parseInt(localStorage.getItem("account_group_page_num")) || 1,
  ...
  table.on("pageLoaded", function(pageNo) {
      localStorage.setItem("account_group_page_num", pageNo);
      localStorage.setItem("account_group_page_size", table.getPageSize());
  });

  // View Account Group
  $(document).on("click", ".view", function (event) {
    const accountGroupId = $(this).data("id");
    const url = accountGroupViewUrl.replace(":id", accountGroupId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Account Group Form...");
  ...
  $(document).on("click", ".edit", function (event) {
    const accountGroupId = $(this).data("id");
    const url = accountGroupEditUrl.replace(":id", accountGroupId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Account Group Form...");
  ...
  $(document).on("click", ".delete", function (event) {
    const accountGroupId = $(this).data("id");
    const url = accountGroupDeleteUrl.replace(":id", accountGroupId);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("account_group_filter");
  if (savedFilter) {
...
function applyFilter() {
  ...
  localStorage.setItem("account_group_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("account_group_filter");
