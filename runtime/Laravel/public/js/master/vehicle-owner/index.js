...
function bindModalEvents() {
  $("#vehicle_owner_modal").on("shown.bs.modal", function () {
    $('#name').focus();
  });

  // Remove focus right before the modal hides to prevent aria-hidden console warnings
  $("#vehicle_owner_modal").on('hide.bs.modal', function () {
...
$(function () {
  ...
  // -----------------------------------
  table = new Tabulator("#vehicle_owner_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("vehicle_owner_table", table);
  }
  ...
  $(document).on("click", ".js-load-modal", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {       
        ...
        bindModalEvents();
        $("#vehicle_owner_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-edit-item", function (event) {
    ...
    const url = vehicleOwnerEditUrl.replace(":id", vehicleOwnerId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Vehicle Owner Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#vehicle_owner_form").attr("data-method", "PUT");
        }, 200);
        $("#vehicle_owner_modal").modal("show");
      },
      error: function (xhr) {
  ...
  $(document).on("click", ".js-view-item", function (event) {
    const vehicleOwnerId = $(this).data("id");
    const url = vehicleOwnerViewUrl.replace(":id", vehicleOwnerId);
    $.ajax({
      url: url,
      beforeSend: function () {
      ...
      success: function (response) {
        ...
        const $modalEl = $("#vehicle_owner_modal");
  ...
  $(document).on("click", ".delete", function (event) {
    const vehicleOwnerId = $(this).data("id");
    const url = vehicleOwnerDeleteUrl.replace(":id", vehicleOwnerId);
