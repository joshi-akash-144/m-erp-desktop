    ...
    // =======================
    {
      ...
      formatter: (cell) => {
        ...
        if (canView) {
          actions += `
            <span 
              data-module="masters/tds-category/modal"
              data-init="openTdsCategoryViewModal"
        ...
        if (canEdit) {
          actions += `
            <span 
              data-module="masters/tds-category/modal"
              data-init="openTdsCategoryEditModal"
...
function bindModalEvents() {
  $("#tds_category_modal").on("shown.bs.modal", function () {
    $('#section').focus();

    const select2Array  = ['type', 'applicable_to'];
...
$(document).ready(function () {
  ...
  $(".js-load-modal").on("click", function (event) {
    $.ajax({
      url: tdsCategoryCreateUrl,
      beforeSend: function () {
        showLoader("Loading Tds Category Form...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        $("#tds_category_modal").modal("show");
      },
      error: function (xhr) {
  ...
  // Account List
  table = new Tabulator("#tds_category_table", {
    dataTree: true,
    ...
    paginationSize: parseInt(localStorage.getItem("tds_category_page_size")) || 50,
    paginationInitialPage: parseInt(localStorage.getItem("tds_category_page_num")) || 1,
  ...
  table.on("pageLoaded", function(pageNo) {
    localStorage.setItem("tds_category_page_num", pageNo);
    localStorage.setItem("tds_category_page_size", table.getPageSize());
  });


    // View tds category
  $(document).on("click", ".view", function (event) {
    const tdsCategoryId = $(this).data("id");
    const url = tdsCategoryViewUrl.replace(":id", tdsCategoryId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Tds Category Form...");
  ...
  $(document).on("click", ".edit", function (event) {
    const tdsCategoryId = $(this).data("id");
    const url = tdsCategoryEditUrl.replace(":id", tdsCategoryId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading Tds Category Form...");
  ...
  $(document).on("click", ".delete", function (event) {
    const tdsCategoryId = $(this).data("id");
    const url = tdsCategoryDeleteUrl.replace(":id", tdsCategoryId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          success: function (response) {
            showToast("success", response.message || "Tds Category successfully!");
            table.replaceData();
          },
          error: function (xhr) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("tds_category_filter");
  if (savedFilter) {
...
function applyFilter() {
  ...
  localStorage.setItem("tds_category_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  ...
  currentFilter = {};
  localStorage.removeItem("tds_category_filter");
