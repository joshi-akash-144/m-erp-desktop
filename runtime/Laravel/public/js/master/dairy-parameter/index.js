...
function bindModalEvents() {
  $("#dairy_parameter_modal").on("shown.bs.modal", function () {

    const select2Array = ['condition_id','element_id'];

    for (let index = 0; index < select2Array.length; index++) {
      const saleType = select2Array[index];  
      $(`#${saleType}`).select2({
        theme: "bootstrap-5",
        dropdownParent: $("#dairy_parameter_modal"),
      });
    }
    $('#condition_id').select2('focus');
  ...
  $("#dairy_parameter_modal").on('hide.bs.modal', function () {
...
function applyFilter() {
  const parameterEl = document.getElementById("condition_id");
  const searchEl = document.getElementById("search");

  const parameterVal = parameterEl?.value || "";
  const searchVal = searchEl?.value.trim() || "";

  currentFilter = {};
  if (parameterVal) currentFilter.filter_parameter_id = parameterVal;
  if (searchVal) currentFilter.search = searchVal;

  // Save filter settings locally
  localStorage.setItem("dairy_parameter_filter", JSON.stringify(currentFilter));
...
function clearFilter() {
  const parameterEl = document.getElementById("parameter_id");
  const searchEl = document.getElementById("search");

  // if (parameterEl?.tomselect) parameterEl.tomselect.setValue("", false);
  if(parameterEl) $("#parameter_id").val("").trigger("change");
  if (searchEl) searchEl.value = "";

  currentFilter = {};
  localStorage.removeItem("dairy_parameter_filter");
...
$(function () {
  ...
  // Load Saved Filters
  // -----------------------------------
  const savedFilter = localStorage.getItem("dairy_parameter_filter");
  if (savedFilter) {
    try {
      currentFilter = JSON.parse(savedFilter);
      if (currentFilter.filter_parameter_id) {
        $elementEl.val(currentFilter.filter_parameter_id).trigger("change");
      }
  ...
  // -----------------------------------
  table = new Tabulator("#dairy_parameter_table", {
  ...
  if (typeof registerTable === "function") {
    registerTable("dairy_parameter_table", table);
  }
  ...
  // Get Modal for create new dairy Parameter
  $(document).on("click", ".js-load-modal", function (event) {
    event.preventDefault();

    $.ajax({
      url: dairyParameterCreateUrl,
      beforeSend: function () {
        showLoader("Loading dairy Parameter Form...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        $("#dairy_parameter_modal").modal("show");
      },
      error: function (xhr) {
  ...
  // Edit dairy Parameter
  $(document).on("click", ".js-edit-item", function (event) {
    const dairyParameterId = $(this).data("id");
    const url = dairyParameterEditUrl.replace(":id", dairyParameterId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading dairy Parameter Form...");
      ...
      success: function (response) {
        ...
        bindModalEvents();
        setTimeout(function () {
          $("#dairy_parameter_form").attr("data-method", "PUT");
        }, 200);
        $("#dairy_parameter_modal").modal("show");
      },
      error: function (xhr) {
  ...
  // View dairy Parameter
  $(document).on("click", ".js-view-item", function (event) {
    const dairyParameterId = $(this).data("id");
    const url = dairyParameterViewUrl.replace(":id", dairyParameterId);
    $.ajax({
      url: url,
      beforeSend: function () {
        showLoader("Loading dairy Parameter Details...");
      },
      success: function (response) {
        $("#global_modal_container").html(response.data.html);
        bindModalEvents();
        ...
        const $modalEl = $("#dairy_parameter_modal");
  ...
  // Delete dairy Parameter
  $(document).on("click", ".delete", function (event) {
    const dairyParameterId = $(this).data("id");
    const url = dairyParameterDeleteUrl.replace(":id", dairyParameterId);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          type: "DELETE",
          beforeSend: function () {
            showLoader("Deleting dairy Parameter...");
          },
          success: function (response) {
            showToast("success", response.message || "dairy Parameter deleted successfully!");
            table.replaceData();
          },
          error: function (xhr) {
            let errMsg = 'Oops! Something went wrong. Try again later.';
