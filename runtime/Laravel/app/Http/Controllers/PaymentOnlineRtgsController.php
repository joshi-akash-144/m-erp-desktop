...
class PaymentOnlineRtgsController extends Controller
{
    ...
    {
        $this->middleware('permission:payment_online_rtgs.list')->only(['index', 'fetchPending']);
        $this->middleware('permission:payment_voucher.delete')->only(['deleteSelected']);
    ...
    public function index()
    {
        ...
        return view('company.pages.payment-online-rtgs.index', compact('banks'));
    }
    ...
    public function saveUpdateChequeNo(Request $request)
    {
        $validated = $request->validate([
            'payment_voucher_ids'   => 'required|array|min:1',
            'payment_voucher_ids.*' => 'required|integer|exists:payment_vouchers,id',
            'chq_name'              => 'required|string|max:255',
            'chq_alpha_number'      => 'required|string|max:150',
            'chq_number'            => 'required|string|max:100',
            'ac_payee'              => 'required|in:yes,no',
            'bank_id'               => 'required',
            'payment_date'          => 'required|date_format:Y-m-d',
        ]);

        try {
            $this->paymentVoucherService->saveUpdateChequeNoForRTGS(
            ...
            return AjaxResponse::success('Cheque No. saved successfully.');
        } catch (Throwable $e) {
            report($e);
            return AjaxResponse::error('Failed to save cheque data: ' . $e->getMessage());
        }
    }
    ...
    public function printRtgs(Request $request)
    {
        ...
        try {
            ...
            return AjaxResponse::success('RTGS print data fetched successfully.', data: ['rtgs_print_html' => $rtgsData['html']]);
        } catch (Throwable $e) {
            report($e);
    ...
    public function deleteSelected(Request $request)
    {
        ...
        try {
            $voucherIds = PaymentVoucher::whereIn('id', $validated['payment_voucher_ids'])
                ->pluck('voucher_id')
                ->toArray();

            foreach ($voucherIds as $voucherId) {
                if ($this->paymentVoucherService->isVoucherLocked($voucherId)) {
                    return AjaxResponse::error(
                        message: 'One or more selected vouchers cannot be deleted because their references have already been settled.',
                        code: 422
                    );
                }
            }

            $this->paymentVoucherService->deleteVoucher($voucherIds);

            return AjaxResponse::success(message: count($voucherIds) . ' payment voucher(s) deleted successfully.');
        } catch (Throwable $e) {
            report($e);
            return AjaxResponse::error(
                message: 'Failed to delete payment vouchers.',
                code: 500,
                errors: $e->getMessage()
            );
        }
