...
class UpdateCreditNoteRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('credit_note.update');
    }
    ...
    public function rules(): array
    {
        ...
        return [
            'credit_note_id'       => ['required', 'integer', Rule::exists('credit_notes', 'id')->where('company_id', $companyId)],
            'credit_note_date'     => ['required', 'date', 'date_format:Y-m-d', new ValidFinancialYearDate($financialYearId)],
            'reference_number'     => [
                'required', 'string', 'max:100',
                Rule::unique('credit_notes', 'reference_number')
                    ->where('company_id', $companyId)
                    ->where('account_id', $this->account_id)
                    ->whereNull('deleted_at')
                    ->ignore($this->credit_note_id),
            ],
            'account_id'           => ['required', 'integer', Rule::exists('accounts', 'id')->where('company_id', $companyId)],
            'sale_type_id'         => ['required', 'integer', Rule::exists('sale_types', 'id')->where('company_id', $companyId)],
            'sales_invoice_id'     => ['nullable', 'integer', Rule::exists('sales_invoices', 'id')->where('company_id', $companyId)],
            'sales_invoice_serial' => ['nullable', 'string', 'max:50'],
    ...
    {
        return [
            'reference_number.unique' => 'This Ref No. is already used for this customer. Please use a different Ref No.',
        ];
    }
