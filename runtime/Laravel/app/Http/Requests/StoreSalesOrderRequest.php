...
class StoreSalesOrderRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('sales_order.create');
    }
    ...
    {
        return [
            'uuid'                      => ['required', 'uuid', 'unique:sales_orders,uuid'],
            'purchase_order_number'     => ['bail', 'required', 'string', 'max:255'],
            'purchase_order_date'       => ['bail', 'required', 'date', 'date_format:Y-m-d'],

            'delivery_date'             => ['bail', 'required', 'date', 'date_format:Y-m-d'], //bail to stop further validation after first failure
    ...
    public function messages(): array
    {
        return [
            ...
            'uuid.unique'                  => 'This sales Invoice already exists.',
