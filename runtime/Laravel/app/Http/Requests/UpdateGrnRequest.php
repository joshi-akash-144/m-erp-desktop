...
class UpdateGrnRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('grn.update');
    }
            ...
            'reference_number' => ['required','string',
                Rule::unique('grns', 'reference_number')
                    ->where(function ($query) use ($companyId, $financialYearId) {
                        return $query->where('company_id', $companyId)->where('account_id', request()->account_id)->where('financial_year_id', $financialYearId);
                    })->ignore($this->route('grn')->id)
            ],
