...
class TransportPaymentData extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:t-payment {company_id} {financial_year_id}';
    ...
    public function setOldData()
    {
        ...
        // }
            ...
            $driverSalary = DB::connection('old_db')
            ->table('driver_salaries')
            ->where('cc_id', 47)
            ->whereNull('deleted_at')
            ->pluck('voucher_no')->toArray();
        ...
        DB::connection('old_db')
            ...
            ->where('cc_id', 47)
            ->whereNull('deleted_at')
            ->whereNotIn('voucher_number', $driverSalary)
                ...
                foreach ($journals as $pv) {
                   ...
                   $voucherSerial = Voucher::where('company_id', $companyId)
                    ...
                    foreach($detialData as $line => $ldData){
                        ...
                        if($ldData->payment_mode==2 && !$drAgianstId){
                            $drAgianstId = $accountMaster[$ldData->account_id] ?? null;
                            if(!$drAgianstId){
                                dd($ldData);
                            }
                        }
                    ...
                    $oldPaymentDetail = $payments->get($paymentId);

                    if(!$oldPaymentDetail) continue;
                    
                    // dd($oldPaymentDetail->cheque_no);
                    if($oldPaymentDetail->cheque_no == null || $oldPaymentDetail->cheque_no == '' || $oldPaymentDetail->amount == null || $oldPaymentDetail->amount == 0) continue;

                    $pmt = Payment::create([
                        'company_id'        => $companyId,
                        'financial_year_id' => $financialYearId,
                        'bank_id' => $accountMaster[$oldPaymentDetail->bank_id] ?? null,
                        'payment_date' => $oldPaymentDetail->payment_date,
                        // 'mode',
                        'amount' => $oldPaymentDetail->amount,
                        'cheque_number' => $oldPaymentDetail->cheque_no,
                        'cheque_date' => $oldPaymentDetail->payment_date,
                        'cheque_time' => $oldPaymentDetail->cheque_time,
                        'cheque_name' =>  null,
                    ]);
