...
class TransportSalaryData extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:t-salary {company_id} {financial_year_id}';
    ...
    public function setOldData()
    {
        ...
        $salaryDetails = DB::connection('old_db')
            ->table('driver_salary_details')
            ->where('cc_id', 47)
        ...
        $driverSalaryAccountId = Account::where('company_id', $companyId)->where('name', 'DRIVER SALARY')->value('id');

        $driverSalaryExpensesId = Account::where('company_id', $companyId)->where('name', 'SALARY EXPENSES')->value('id');

        DB::connection('old_db')
            ->table('driver_salaries')
            ->where('cc_id', 47)
            ->whereNull('deleted_at')

            ->orderBy('id')
            ->chunk(200, function ($salary) use (
                $companyId,
                $financialYearId,
                $accountMaster,
                $type,
                ...
                $bhatthu,
                $vehicles,
                $driverSalaryAccountId,
                $driverSalaryExpensesId
            ) {
                // $voucherSerial = 5000;
                // $journalVoucherSerial = 5000;
                foreach ($salary as $key => $slr) {



                    $details = $salaryDetails[$slr->voucher_no];
                    // dd($slr);
                    $month = Carbon::parse($slr->month)->format('F');
                    $voucherDate = $slr->voucher_date;
                    $salaryDate = $slr->expence_date;

                    
                    // base Account = DRIVER SILAK 
                    
                    foreach ($details as $detail) {
                        $newVehicleId = $vehicleMapping[$detail->vehicle_id];
                        $driverId = $accountMaster[$detail->driver_id] ?? null;
                        if(!$driverId){
                            if($detail->driver_id == 4543 || $detail->driver_id == 4544) continue;
                            dd($detail);
                        }
                        ...
                        //     'narration'         => 'Old Payment Voucher #' . $detail->voucher_no . ', Salary Month : ' . $month . ', Vehicle No: ' . $vehicles[$newVehicleId] . ', Salary Month Date: ' . $salaryDate,
                        ...
                        //         'against_account_id' => $driverSalaryAccountId,
                        //         'narration'          => null,
                        //         'line_no'            => 1,
                        //         'created_at'         => $now,
                        //         'updated_at'         => $now,
                        ...
                        //     [
                        //         'voucher_id'         => $voucher->id,
                        //         'account_id'         => $driverSalaryAccountId,
                        //         'debit'              => 0,
                        //         'credit'             => $detail->salary,
                        ...
                        //  Same time post Journal Voucher
 ...
 $voucherSerial = Voucher::where('company_id', $companyId)
                    ...
                    $voucherSerial++;
                        ...
                        $jvVoucher =  Voucher::create([
                            ...
                            'reference_number'  => null,
                            'narration'         => 'Salary Month : ' . $month . ', Vehicle No: ' . $vehicles[$newVehicleId] . ', Salary Month Date: ' . $salaryDate,
                        ]);
                        ...
                        VoucherTransaction::insert([
                            [
                                ...
                                'account_id'         => $driverSalaryExpensesId,
                                'debit'              => $detail->salary,
                                'credit'             => 0,
                                'is_party_account'   => false,
                                'against_account_id' => $driverId,
                            ...
                            [
                                ...
                                'credit'             => $detail->salary,
                                'is_party_account'   => true,
                                'against_account_id' => $driverSalaryExpensesId,
                                'narration'          => null,
                                'line_no'            => 2,
                                'created_at'         => $now,
                                'updated_at'         => $now,
                       ...
                       $jv =  JournalVoucher::create([
                        ...
                        VehicleExpense::create([
                            ...
                            'financial_year_id' => $financialYearId,
                            'vehicle_id' => $newVehicleId,
                            'expense_account_id' => $driverSalaryExpensesId,
