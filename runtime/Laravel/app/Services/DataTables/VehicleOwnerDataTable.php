...
class VehicleOwnerDataTable
{
    ...
    public function getData($query): JsonResponse
    {
        ...
        if ($request->has('filter_vehicle_owner_id') && $request->input('filter_vehicle_owner_id') !== '') {
            $query->where('id', $request->input('filter_vehicle_owner_id'));
        }
