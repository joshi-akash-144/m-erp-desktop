...
class PurchaseTypeDataTable
{
    ...
    public function getData($query): JsonResponse
    {
        ...
        // --- Filters ---
        if ($request->filled('filter_type_id')) {
            $query->where('taxation_type', $request->input('filter_type_id'));
        }
        ...
        $rows = $query
            ->with('account:id,name','creator:id,name', 'updater:id,name')
            ->skip($start)
            ->take($size)
            ->get();
