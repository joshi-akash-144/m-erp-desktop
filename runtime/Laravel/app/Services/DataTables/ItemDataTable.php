...
class ItemDataTable
{
    ...
    public function getData(Builder $query): JsonResponse
    {
        $request = $this->request;

        // --- Filters ---
        if ($request->filled('filter_group_id')) {
            $query->where('item_group_id', $request->input('filter_group_id'));
        }
        ...
        $rows = $query
            ->with('itemGroup:id,name','unit:id,name','taxCategory:id,name','creator:id,name','updater:id,name','currentOpeningStock')
            ->skip($start)
            ->take($size)
            ->get();
