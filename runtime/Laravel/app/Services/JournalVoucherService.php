...
class JournalVoucherService
{
    ...
    /*--------------------------------------------------------------
    | VALIDATE EXPENSE ROWS (when vehicle is set)
    --------------------------------------------------------------*/
    private function validateExpenseRows(array $rows): void
    {
        foreach ($rows as $index => $row) {
            if (strtoupper($row['dr_cr'] ?? '') !== 'DR') continue;

            $account = Account::select('account_group_id')->find($row['account_id']);
            if (!$account) continue;

            $group = AccountGroup::select('type')->find($account->account_group_id);
            if (!$group || $group->type !== 'expense') {
                $rowNum = $index + 1;
                throw new \Exception("Row {$rowNum}: When a vehicle is selected, all Debit entries must belong to an Expense group.");
            }
        }
    }
    ...
    public function createVoucher(array $data, $companyId, $financialYearId)
    {
        ...
        if (!empty($data['vehicle_id'])) {
            $this->validateExpenseRows($data['rows'] ?? []);
        }
        ...
        return DB::transaction(function () use ($data, $companyId, $financialYearId) {
            ...
            if (!empty($data['vehicle_id'])) {
                $this->syncVehicleExpenses($data, $voucher->id, $companyId, $financialYearId);
            }

            if (!empty($data['tds_details'])) {
                $this->saveTdsEntry($data['tds_details'], $voucher, $companyId, $financialYearId, $data['voucher_date']);
            }
    ...
    public function prepareVoucherPayload(array $data, $companyId, $financialYearId)
    {
        ...
        return [
            ...
            'journal' => $this->prepareJournal($data, $serialInfo, $companyId, $financialYearId)
        ];
    }


    private function prepareJournal($data, $serialInfo, $companyId, $financialYearId)
    {
        return [
            'company_id'        => $companyId,
            'financial_year_id' => $financialYearId,
    ...
    private function prepareVoucherData(array $data, $serialInfo, $companyId, $financialYearId)
    {
        return [
            ...
            'reference_id'     => null,
            'reference_type'   => Reference::Journal,
    ...
    | 4.5. SAVE TDS ENTRY
    --------------------------------------------------------------*/
    private function saveTdsEntry(array $tdsDetails, $voucher, $companyId, $financialYearId, $voucherDate)
    {
        $party = Account::find($tdsDetails['party_id']);
        
        TdsEntry::create([
            'company_id'        => $companyId,
            'financial_year_id' => $financialYearId,
            'voucher_id'        => $voucher->id,
            'account_id'        => $tdsDetails['party_id'],
            'tds_category_id'   => $tdsDetails['tds_category_id'],
            'deductee_name'     => $party ? $party->name : null,
            'pan_no'            => $tdsDetails['pan_no'] ?? ($party ? $party->pan_no : null),
            'reference_no'      => $tdsDetails['reference_no'] ?? null,
            'payment_amount'    => $tdsDetails['assessable_value'],
            'payment_date'      => $voucherDate,
            'tds_rate'          => $tdsDetails['percentage'],
            'tds_amount'        => $tdsDetails['tds_amount'],
            'tax_deducted_on'   => $voucherDate,
            'voucher_type_id'   => VoucherType::JOURNAL,
        ]);
    }
    ...
    {
        $voucherTypeId = VoucherType::JOURNAL;
        $offset = ($page - 1) * $size;
        ...
        $permissions = userPermissions([
            'journal_voucher.view',
            'journal_voucher.update',
            'journal_voucher.delete',
            'journal_voucher.print-voucher',
        ], true);
    ...
    public function details($id)
    {
        ...
        $journal = JournalVoucher::withTrashed()->where('voucher_id', $id)->first();
        $tdsEntry = TdsEntry::where('voucher_id', $id)->first();
    ...
    public function voucherSerials(int $companyId, int $financialYearId){
        ...
        return  Voucher::where('company_id', $companyId)
            ...
            // ->where('journal_entry_from', JournalVoucher::ENTRY_FORM_VOUCHER)
    ...
    public function updateVoucher($voucherId, array $data, $companyId, $financialYearId)
    {
        ...
        return DB::transaction(function () use ($voucherId, $data, $companyId, $financialYearId) {
            ...
            $journal = JournalVoucher::where('voucher_id', $voucherId)->firstOrFail();
             ...
             */
            $journal->update([
                'gst_nature' => $data['gst_nature'] ?? 'gst_not_applicable',
                'vehicle_id' => $data['vehicle_id'] ?? null,
                'bill_date'  => $data['bill_date']  ?? null,
            ]);
            ...
            /*
             * STEP 4.5 — Sync TDS entries.
             */
            TdsEntry::where('voucher_id', $voucherId)->delete();
            if (!empty($data['tds_details'])) {
                $this->saveTdsEntry($data['tds_details'], $voucher, $companyId, $financialYearId, $data['voucher_date']);
            }

            /*
             * STEP 5 — Sync vehicle expense rows.
             */
            VehicleExpense::where('voucher_id', $voucherId)->delete();
            if (!empty($data['vehicle_id'])) {
                $this->syncVehicleExpenses($data, $voucherId, $companyId, $financialYearId);
            }
    ...
    public function deleteVoucher(int $voucherId): void
    {
        ...
        DB::transaction(function () use ($voucherId) {
            ...
            $journalVoucher = JournalVoucher::where('voucher_id', $voucherId)->first();

            $data['old_values'] = [
                'voucher_date'     => $voucher->voucher_date,
                'narration'        => $voucher->narration,
                'gst_nature'       => $journalVoucher->gst_nature ?? null,
                'vehicle_id'       => $journalVoucher->vehicle_id ?? null,
            ...
            // We do not physically delete VoucherTransaction or VehicleExpense here.
    ...
    /*--------------------------------------------------------------
    | SYNC VEHICLE EXPENSES — creates one row per DR expense line
    --------------------------------------------------------------*/
    private function syncVehicleExpenses(array $data, int $voucherId, $companyId, $financialYearId): void
    {
        foreach ($data['rows'] as $row) {
            if (strtoupper($row['dr_cr'] ?? '') !== 'DR') continue;
            $amount = (float) ($row['debit_amount'] ?? 0);
    ...
    private function logAudit($data, $voucher, $action)
    {
        ...
        $auditData = [
            ...
            'action'            => $action,
            'module'            => SourceType::JOURNAL,
            'record_type'       => AuditTrail::RECORD_TYPE_VOUCHER,
            'model_name'        => JournalVoucher::class,
