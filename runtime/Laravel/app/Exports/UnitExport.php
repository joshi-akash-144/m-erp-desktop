...
class UnitExport implements FromView, WithEvents
{
    protected $company;
    protected $units;
    protected $headings;
    ...
    public function __construct($company, $units, $headings)
    {
    ...
    {
        $rows = $this->units->map(function ($unit) {
            return [
                $unit->name,
                $unit->print_name,
                $unit->uqc,
       ...
       $this->totalColumn = $rows->isNotEmpty() ? count($rows->first()) : 0;
        ...
        return view('company.pages.masters.unit.export', [
