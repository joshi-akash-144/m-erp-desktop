...
class DairyAnalysisRebatePendingExport implements FromView, WithEvents
{
    protected $company;
    protected $rows;
    protected $headings;
    ...
    {
        return view('company.pages.dairy-analysis.rebate_pending_export', [
            'companyName' => $this->company->print_name ?? $this->company->name,
            'gstNumber'   => $this->company->gst_number,
            'reportTitle' => 'Dairy Analysis Rebate Pending',
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
               ...
               $widths = [];
                ...
                // ====== Alignment for Centered Columns ======
                foreach (['E', 'G', 'I', 'J', 'K', 'L', 'M', 'P'] as $col) {
                    $sheet->getStyle("{$col}5:{$col}{$lastDataRow}")
                        ->getAlignment()
                        ->setHorizontal(Alignment::HORIZONTAL_CENTER);
                }

                // ====== Alignment for Right Columns ======
                foreach (['A', 'H', 'O'] as $col) {
                    $sheet->getStyle("{$col}5:{$col}{$totalRow}")
                        ->getAlignment()
                        ->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                }

                // Format quantity columns to 2 decimal places
                foreach (['H', 'O'] as $col) {
