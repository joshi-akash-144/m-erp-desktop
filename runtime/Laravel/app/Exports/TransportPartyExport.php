...
class TransportPartyExport implements FromView, WithEvents
{
    protected $company;
    protected $records;
    protected $headings;
    ...
    public function __construct($company, $records, $headings)
    {
    ...
    {
        $rows = $this->records->map(function ($record) {
            return [
                $record->name ?? '',
                $record->state->name ?? '',
                $record->city ?? '',
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                for ($i = 1; $i <= $totalColumn; $i++) {
                    ...
                    $widths[$i] = in_array($columnLetter, ['A', 'G', 'H']) ? 30 : 20;
                }
