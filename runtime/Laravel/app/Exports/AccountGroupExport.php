...
class AccountGroupExport implements FromView, WithEvents
{
    /**
     ...
     */
    protected \Illuminate\Support\Collection $accountGroups;
     ...
     * @param \Illuminate\Support\Collection $accountGroups
     * @param array $headings
     */
    public function __construct(object $company, \Illuminate\Support\Collection $accountGroups, array $headings)
    {
    ...
    {
        $rows = $this->accountGroups->map(function ($accountGroup) {
            return [
                $accountGroup->name,
                $accountGroup->parent_id ? ($accountGroup->parent->name ?? 'Unknown') : 'Primary Account',
                $accountGroup->type ? ucfirst($accountGroup->type) : '-',
            ];
        });

        $this->totalColumn = $rows->isNotEmpty() ? count($rows->first()) : 0;

        return view('company.pages.masters.account-group.export', [
