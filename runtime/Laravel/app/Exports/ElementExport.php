...
class ElementExport implements FromView, WithEvents
{
    protected $company;
    protected $elements;
    protected $headings;
    ...
    public function __construct($company, $elements, $headings)
    {
    ...
    {
        $rows = $this->elements->map(function ($element) {
            return [
                $element->name,
                $element->print_name,
                $element->range,
       ...
       $this->totalColumn = $rows->isNotEmpty() ? count($rows->first()) : 0;
        ...
        return view('company.pages.masters.element.export', [
            'companyName' => $this->company->print_name,
            'gstNumber'       => $this->company->gst_number,
            'reportTitle' => 'Element Report',
