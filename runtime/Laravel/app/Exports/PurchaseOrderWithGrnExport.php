...
class PurchaseOrderWithGrnExport implements FromView, WithEvents
{
    ...
    public function __construct(object $company, \Illuminate\Support\Collection $purchaseOrders, array $headings, array $footerData = [], ?string $datePeriod = null)
    {
    ...
    {
        $rows = $this->purchaseOrders->map(function ($row) {
            return [
                $row['po_no'] ?? '',
                $row['po_date'] ?? '',
                $row['supplier_name'] ?? '',
        ...
        return view('company.pages.purchase-order-with-grn.export', [
            'companyName' => $this->company->print_name,
            'gstNumber' => $this->company->gst_number,
            'reportTitle' => 'Purchase Order Detail With GRN Report',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $firstDataRow = 6;
                $rowsCount = $this->purchaseOrders->count() + (empty($this->footerData) ? 0 : 1);
