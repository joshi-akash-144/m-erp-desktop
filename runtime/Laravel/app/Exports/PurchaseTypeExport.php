...
class PurchaseTypeExport implements FromView, WithEvents
{
    /**
     ...
     */
    protected \Illuminate\Support\Collection $purchaseTypes;
     ...
     * @param \Illuminate\Support\Collection $purchaseTypes
     * @param array $headings
     */
    public function __construct(object $company, \Illuminate\Support\Collection $purchaseTypes, array $headings)
    {
    ...
    {
        $rows = $this->purchaseTypes->map(function ($purchaseType) {
            return [
                $purchaseType->name,
                $purchaseType?->account->name,
                config('constants.purchase_taxation_types')[$purchaseType->taxation_type] ?? '',
                config('constants.purchase_transaction_types')[$purchaseType->transaction_type] ?? '',
                config('constants.purchase_regions')[$purchaseType->region] ?? '',
                $purchaseType->cgst,
                $purchaseType->sgst,
                $purchaseType->igst,
            ];
        ...
        return view('company.pages.masters.purchase-type.export', [
            'companyName' => $this->company->print_name,
            'gstNumber'   => $this->company->gst_number,
            'reportTitle' => 'Purchase Type Report',
