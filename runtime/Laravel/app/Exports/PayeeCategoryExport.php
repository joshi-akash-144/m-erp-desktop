...
class PayeeCategoryExport implements FromView, WithEvents
{
    protected $company;
    protected $payeeCategory;
    protected $headings;
    protected $totalColumn;

    public function __construct($company, $payeeCategory, $headings)
    {
    ...
    {
        $rows = $this->payeeCategory->map(function ($payeeCate) {
            return [
                $payeeCate->payee_category,
            ];
        });

        $this->totalColumn = $rows->isNotEmpty() ? count($rows->first()) : 0;

        return view('company.pages.masters.payee-category.export', [
            'companyName' => $this->company->print_name,
            'gstNumber'       => $this->company->gst_number,
            'reportTitle' => 'Payee Category Report',
