...
class ConditionExport implements FromView, WithEvents
{
    ...
    public function __construct($company, $conditions, $headings)
    {
    ...
    {
        $rows = $this->conditions->map(function ($condition) {
            return [
                $condition->name,
                $condition->print_name,
            ];
        });

       $this->totalColumn = $rows->isNotEmpty() ? count($rows->first()) : 0;

        return view('company.pages.masters.condition.export', [
            'companyName' => $this->company->print_name,
            'gstNumber'       => $this->company->gst_number,
            'reportTitle' => 'Conditions Report',
