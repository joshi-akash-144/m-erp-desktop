...
class TdsCategoryExport implements FromView, WithEvents
{
    protected $company;
    protected $tdsCategory;
    protected $headings;
    protected $totalColumn;

    public function __construct($company, $tdsCategory, $headings)
    {
    ...
    {
        $rows = $this->tdsCategory->map(function ($tdsCate) {
            return [
                $tdsCate->category_name,
                $tdsCate->section,
                $tdsCate->rate,
                $tdsCate->applicable_to,
                $tdsCate->type,
                $tdsCate->description,               
            ];
        });
       ...
       $this->totalColumn = $rows->isNotEmpty() ? count($rows->first()) : 0;
        ...
        return view('company.pages.masters.tds-category.export', [
            'companyName' => $this->company->print_name,
            'gstNumber'       => $this->company->gst_number,
            'reportTitle' => 'TDS Category Report',
            ...
            AfterSheet::class => function (AfterSheet $event) {
               ...
               $widths = [];
                ...
                for ($i = 1; $i <= $totalColumn; $i++) {
                    ...
                    if (in_array($columnLetter, ['A'])) {
                        $widths[$i] = 25;
                    } elseif (in_array($columnLetter, ['B', 'C', 'E'])) {
                        $widths[$i] = 15;
                    } elseif (in_array($columnLetter, ['F'])) {
                        $widths[$i] = 60;
                    } elseif (in_array($columnLetter, ['D'])) {
                        $widths[$i] = 30; 
                    }
                }
