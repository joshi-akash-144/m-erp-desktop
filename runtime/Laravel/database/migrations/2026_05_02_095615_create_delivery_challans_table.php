...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('delivery_challans', function (Blueprint $table) {
           ...
           // Challan Info
            $table->bigInteger('challan_serial');
            ...
            $table->date('challan_in_date')->nullable();
            $table->date('challan_out_date')->nullable();
            ...
            $table->enum('qc_status', ['pending', 'passed', 'failed'])->default('passed');

            // Remarks
            $table->text('remarks')->nullable();

            // Challan Status
            $table->enum('challan_status', [
                'open',        // Challan created
                'hold',        // Waiting for QC / approval
                'close',       // Finalized but not billed
                'billed',      // Sales Order created
                'cancel',      // Cancelled
            ...
            $table->unique(['company_id', 'financial_year_id', 'challan_serial', 'deleted_at'], 'unique_challan_serial');
            $table->unique(['company_id', 'financial_year_id', 'challan_number', 'deleted_at'], 'unique_challan_number');
